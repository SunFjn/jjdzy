  ...
  $('.goplay').click(function () {
    ...
    if (state == 0 && !whitelist) {
      ...
      if ($("#alert").length <= 0) {
        ...
        var data = {};
        var time = Math.random();
        data.sign = md5("cmd=" + 203 + "ip=" + loginArg.ip + "openid=" + loginArg.account + "pf=" + loginArg.pf + "randnum=" + time + "zoneid=" + loginArg.zoneid + "clientKey");
        data.cmd = 203;
        data.pf = loginArg.pf;
        data.zoneid = loginArg.zoneid;
        data.ip = loginArg.ip;
        data.openid = loginArg.account;
        data.randnum = time;
...
function runEgretMain() {
  if (isClickEnter && isLoadMain) {
    var serverData = {};
    serverData.ip = loginArg.ip;
    serverData.port = loginArg.port;
    serverData.zoneid = loginArg.zoneid;
    serverData.alias = loginArg.serverName;
    serverData.state = loginArg.state;

...
function showgonggao(e) {
  if (loginArg) {
    ...
    if (!isGG) {
      ...
      if (isDebugModel) {
        url = "loginjs/brocastServer.php?" + Math.random();
      }
  ...
  if (str == "最近登录") {
    if (recentServers) {
      for (var i = recentServers.length - 1; i >= 0; i--) {
        ...
        server_div += GetServerSelDiv(data.alias, data.state, data.ip, data.port, data.zoneid, data.clientversion, data.platform);
      }
    }
  } else {
    var sp = str.split('-');
...
function GetServerSelDiv(servername, status, ip, port, zoneid, clientversion, pf) {
  var tag = "<span ></span>";
  ...
  return '<div ip=' + ip + ' port=' + port + ' alias=' + servername + ' zoneid=' + zoneid + ' state=' + status + ' clientversion=' + clientversion + " pf=" + pf + ' class="servernoselect">' + servername + tag + '</div>';
}
...
function bodyOnLoad() {
  ...
  var mainurl = '../manifest.json?v=' + Math.random();
...
function runEgretMain() {
  if (isClickEnter && isLoadMain) {
    ...
    loginArg.pf = serverData.pf = pf;
    loginArg.clientversion = serverData.clientversion = clientversion;

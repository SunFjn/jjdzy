...
function testInitData() {//内测服走此处。
  if (loginArg) {
    ...
    loadHttp(url, function (result) {
      ...
      alias = selectD.alias;
      state = selectD.state;
      loginArg.clientversion = selectD.clientversion;
      loginArg.resversion = selectD.resversion;
      setServerSt(state);
...
function updateSerList(str) {//显示服务器列表
  ...
  if (str == "最近登录") {
    if (recentServers) {
      for (var i = recentServers.length - 1; i >= 0; i--) {
        ...
        server_div += GetServerSelDiv(data.alias, data.state, data.ip, data.port, data.zoneid, data.clientversion, data.resversion);
      }
    }
  } else {
    var sp = str.split('-');
  ...
  $('.serversel>div').click(function (e) {
    ...
    resversion = $(this).attr('resversion');
    loginArg.clientversion = clientversion;
    loginArg.resversion = resversion;
    if (state == 0) {
...
function GetServerSelDiv(servername, status, ip, port, zoneid, clientversion,resversion) {
  var tag = "<span ></span>";
  ...
  return '<div ip=' + ip + ' port=' + port + ' alias=' + servername + ' zoneid=' + zoneid + ' state=' + status + ' clientversion=' + clientversion+ ' resversion=' + resversion  + ' class="servernoselect">' + servername + tag + '</div>';
}

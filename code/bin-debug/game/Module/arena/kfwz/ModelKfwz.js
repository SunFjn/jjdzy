 ...
 */
var ModelKfwz = (function (_super) {
    __extends(ModelKfwz, _super);
    function ModelKfwz() {
        ...
        _this.buyCount = 0;
        /** 队伍信息 */
        _this.teamVo = new VoTeamKfwz();
        /** 我的战斗队伍 */
        _this.myBattleList = [];
        /** 敌方战斗队伍 */
        _this.enemyBattleList = [];
        _this.battleVoMap = {};
        /** 我方战斗顺序列表（会因死亡状态而改变顺序） */
        _this.myFightList = [];
        /** 敌方战斗顺序列表（会因死亡状态而改变顺序） */
        _this.enemyFightList = [];
        _this.teamList = [];
        /** 匹配状态中 */
        _this.isMatching = false;
        /** 结束时候发送等待标识 */
        _this.overWaiting = false;
        /** 战斗类型 0人 1电脑 */
        _this.battleType = 0;
        /** 是否在活动中 */
        _this.isInAct = false;
        ...
        _this._curZsId = -1;
        _this._rangeId = 0;
        _this._targetVoListMap = {};
        _this._gradeListMap = {};
        _this._battleRewardVoMap = {};
        /** 自动开始标识 */
        _this.autoStartFlag = false;
        /** 自动开始倒计时（s） */
        _this.autoStartTime = 5;
        return _this;
    }
    ModelKfwz.prototype.setup = function () {
        if (this._setupFlag)
            return;
        this._setupFlag = true;
        {
            var t_cfg = Config.kfwzmb_770;
            for (var k in t_cfg) {
                var t_vo = new VoTargetKfwz();
                t_vo.id = ~~k;
    ...
    //========================================= 协议相关 ========================================
    //协议处理
    ModelKfwz.prototype.listenServ = function (mgr) {
        this.socket = mgr;
        //注册GC方法
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_actstate_10820 = function (self, data) {
        self.setup();
        var t_change = false;
    ...
    ModelKfwz.prototype.cmdSendEnter = function () {
        var t = this;
        if (t.isInAct)
            return;
        t.CG_CrossTeamKing_joinAct_10859();
    };
    ModelKfwz.prototype.cmdSendExit = function () {
        var t = this;
        if (!t.isInAct)
            return;
    ...
    /**10821  打开ui  */
    ModelKfwz.prototype.CG_CrossTeamKing_openUi_10821 = function () {
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_openUi_10822 = function (self, data) {
        var t_change = false;
        var arg1 = data.readByte(); //活动状态
        var arg2 = data.readByte(); //转生区间
        var arg3 = data.readByte(); //段位
        var arg4 = data.readInt(); //赛季积分
        var arg5 = data.readInt(); //赛季排名
        var arg6 = data.readInt(); //胜利场数
        var arg7 = data.readInt(); //剩余挑战次数
        var arg8 = data.readInt(); //已购买次数
        arg3 = arg3 < 1 ? 1 : arg3;
        var len = data.readShort();
        for (var i = 0; i < len; i++) {
            var arg9 = data.readInt(); //宝箱id
            var arg10 = data.readByte(); //宝箱领取状态
            var t_vo = self.getTargetVoById(arg9);
            if (t_vo && t_vo.update({ state: arg10 })) {
                t_change = true;
            }
        }
        var arg11 = data.readInt(); //当前连胜数
    ...
    ModelKfwz.prototype.updateInfo = function (pData) {
        var t = this;
        return ObjectUtils.modifyObject(t, pData);
    };
    /**10823  CG 创建队伍（进入跨服） */
    ModelKfwz.prototype.CG_CrossTeamKing_createteam_10823 = function () {
        var bates = this.getBytes();
        this.sendSocket(10823, bates, true);
    };
    /**10824 B-I GC 创建队伍返回 B:0成功 1已经存在 2活动未开启 3次数不够restI:队伍idteamid*/
    ModelKfwz.prototype.GC_CrossTeamKing_createteam_10824 = function (self, data) {
        var arg1 = data.readByte();
    ...
    /**10825  CG 邀请玩家参与（跨服） */
    ModelKfwz.prototype.CG_CrossTeamKing_invitation_10825 = function () {
        var t = this;
        if (!t.isInTeam)
            return;
        if (!t.areYouLeader)
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_invitation_10826 = function (self, data) {
        var arg1 = data.readByte();
        switch (arg1) {
            case 0:
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_invitationBroads_10828 = function (self, data) {
        var arg1 = data.readInt();
        var arg2 = data.readUTF();
    };
    ModelKfwz.prototype.cmdSendJoinTeam = function (pTeamId) {
        var t = this;
        if (t.isInAct) {
            //正在跨服中
    ...
    //用于超时清除缓存的队伍id
    ModelKfwz.prototype.onDelayClearTeamId = function () {
        var t = this;
        t._tempToJoinTeamId = 0;
    };
    /**10829 I CG 申请加入某个队伍 I:队伍idteamid*/
    ModelKfwz.prototype.CG_CrossTeamKing_joinTeam_10829 = function (pTeamId) {
        var t = this;
        if (!t.actState) {
            ViewCommonWarn.text("活动尚未开启");
            t._tempToJoinTeamId = 0;
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_joinTeam_10830 = function (self, data) {
        var arg1 = data.readByte();
        switch (arg1) {
            case 0:
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_teamInfoChage_10832 = function (self, data) {
        var t_change = false;
        var arg1 = data.readByte(); //操作字段:0创建队伍1有加入2有人退出
        ...
        var len = data.readShort();
        for (var i = 0; i < len; i++) {
            var arg4 = data.readLong(); //玩家id
            var arg5 = data.readInt(); //头像
            var arg6 = data.readInt(); //头像框
            var arg7 = data.readUTF(); //玩家名字
            var arg8 = data.readInt(); //等级
            var arg9 = data.readLong(); //战力
            var t_memberVo = new VoTeamMemberKfwz();
    ...
    ModelKfwz.prototype.CG_CrossTeamKing_exchange_10833 = function (pIndex1, pIndex2) {
        var t = this;
        if (!t.isInTeam)
            return false;
        if (!t.areYouLeader)
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_exchange_10834 = function (self, data) {
        var arg1 = data.readByte();
    ...
    ModelKfwz.prototype.CG_CrossTeamKing_exitteam_10835 = function () {
        var t = this;
        if (!t.isInTeam)
            return;
        var bates = this.getBytes();
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_moveteam_10836 = function (self, data) {
        var arg1 = data.readByte();
        switch (arg1) {
            case 0://成功退出队伍
                self.clearTeamVo();
    ...
    /** 清空队伍信息 */
    ModelKfwz.prototype.clearTeamVo = function () {
        var t = this;
        if (t.isInTeam) {
            t.teamVo.clear();
    ...
    ModelKfwz.prototype.CG_CrossTeamKing_moveMeber_10837 = function (pIndex) {
        var t = this;
        if (!t.isInTeam)
            return;
        if (!t.areYouLeader)
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_moveMeber_10838 = function (self, data) {
        var arg1 = data.readByte();
        switch (arg1) {
            case 0://移除成功
                if (self.isInTeam) {
    ...
    /**10855  CG 取消匹配战斗（跨服） */
    ModelKfwz.prototype.CG_CrossTeamKing_cancelMarry_10855 = function () {
        var t = this;
        if (!t.isInTeam)
            return;
        if (!t.areYouLeader)
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_cancelMarry_10856 = function (self, data) {
        var arg1 = data.readByte();
        switch (arg1) {
            case 0:
                self.isMatching = false;
    ...
    /**10839 （跨服）  CG 开始匹配战斗 */
    ModelKfwz.prototype.CG_CrossTeamKing_marryBattle_10839 = function () {
        var t = this;
        if (!t.isInTeam)
            return;
        if (!t.areYouLeader)
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_marryBattle_10840 = function (self, data) {
        var arg1 = data.readByte();
        switch (arg1) {
            case 0://开始匹配
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_battleUi_10842 = function (self, data) {
        self.isMatching = false;
        self.battleType = 0;
        self.overWaiting = false;
        GGlobal.layerMgr.close2(UIConst.KFWZ_MATCH); //关闭匹配界面
        var t_hasCPU = false; //是否有电脑
        self.clearBattleList(); //清空战斗队伍信息
        {
            var len = data.readShort();
            for (var i = 0; i < len; i++) {
                var arg1 = data.readInt(); //头像
                var arg2 = data.readInt(); //头像框
                var arg3 = data.readUTF(); //玩家名字
                var arg4 = data.readLong(); //战力
                var arg5 = data.readByte(); //对手类型 0人 1电脑
                var arg6 = data.readLong(); //玩家id
                var t_vo = VoBattlePlayerKfwz.getFromPool();
                t_vo.roleId = arg6;
                t_vo.head = arg1;
                t_vo.headGrid = arg2;
                t_vo.name = arg3;
        ...
        }
        {
            ...
            for (var i = 0; i < len; i++) {
                ...
                var arg9 = data.readUTF();
                var arg10 = data.readLong();
    ...
    /** 对战斗顺序重新排序 */
    ModelKfwz.prototype.sortFightList = function () {
        var t = this;
        t.myFightList.sort(function (pA, pB) {
            return pB.sortValue - pA.sortValue;
        });
        t.enemyFightList.sort(function (pA, pB) {
            return pB.sortValue - pA.sortValue;
        });
    };
    ModelKfwz.prototype.clearBattleList = function () {
        var t = this;
        for (var i = t.myBattleList.length - 1; i >= 0; i--) {
            var t_vo = t.myBattleList[i];
            delete t.battleVoMap[t_vo.roleId];
            VoBattlePlayerKfwz.recycleToPool(t_vo);
            t.myBattleList.splice(i, 1);
        }
        t.myFightList.length = 0;
        for (var i = t.enemyBattleList.length - 1; i >= 0; i--) {
            var t_vo = t.enemyBattleList[i];
            delete t.battleVoMap[t_vo.roleId];
            VoBattlePlayerKfwz.recycleToPool(t_vo);
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_battleinfo_10844 = function (self, data) {
        var arg1 = data.readInt(); //队伍id
        var arg2 = data.readInt(); //死亡的队员index
        var t_battleList;
    ...
    /** 更新战斗死亡状况 */
    ModelKfwz.prototype.updateBattleDead = function (pRoleId) {
        var t = this;
        var t_vo = t.getBattleVoByRoleId(pRoleId);
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_uiupdate_10846 = function (self, data) {
        var t_change = false;
        var arg1 = data.readByte(); //段位
        var arg2 = data.readInt(); //赛季积分
        var arg3 = data.readInt(); //赛季排名
        var arg4 = data.readInt(); //胜利场数
        var arg5 = data.readInt(); //剩余挑战次数
    ...
    ModelKfwz.prototype.CG_CrossTeamKing_openRank_10847 = function (arg1) {
        var bates = this.getBytes();
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_openRank_10848 = function (self, data) {
        var t_change = false;
        var arg1 = data.readByte(); //转生区间
        var arg2 = data.readInt(); //我的排名
        var arg3 = data.readInt(); //我的段位
        var t_myRangeId = self.getRangeId();
        if (arg1 == t_myRangeId) {
            if (self.myRank != arg2) {
                self.myRank = arg2;
                t_change = true;
        ...
        var t_rankVoList = self.getRankCfgListByRangeId(arg1);
        var t_checkMap = {};
        var len = data.readShort();
        for (var i = 0; i < len; i++) {
            var arg4 = data.readInt(); //排名
            var arg5 = data.readInt(); //段位
            var arg6 = data.readUTF(); //名字
            var arg7 = data.readInt(); //积分
            var t_vo = t_rankVoList[arg4 - 1];
            if (t_vo) {
                //存在则修改原有数据
    ...
    ModelKfwz.prototype.CG_CrossTeamKing_getReward_10849 = function (arg1) {
        var bates = this.getBytes();
        bates.writeInt(arg1);
        this.sendSocket(10849, bates);
    };
    /**10850 I-B GC 奖励变化 I:奖励索引indexB:奖励状态0 1 2state*/
    ModelKfwz.prototype.GC_CrossTeamKing_reward_10850 = function (self, data) {
        var t_change = false;
        var arg1 = data.readInt();
        var arg2 = data.readByte();
    ...
    /**10851  CG 查询战报（跨服） */
    ModelKfwz.prototype.CG_CrossTeamKing_getLog_10851 = function () {
        var bates = this.getBytes();
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_returnLog_10852 = function (self, data) {
        self.clearLogList();
        var len = data.readShort();
        for (var i = 0; i < len; i++) {
            var arg1 = data.readByte(); //胜负1胜2负
            var arg2 = data.readInt(); //获得积分
            var t_voLog = VoLogKfwz.getFromPool();
            t_voLog.result = arg1;
            t_voLog.score = arg2;
            {
                var len1 = data.readShort();
                for (var i_1 = 0; i_1 < len1; i_1++) {
                    var arg3 = data.readLong(); //玩家id
                    var arg4 = data.readByte(); //是否队长
                    var arg5 = data.readInt(); //头像
                    var arg6 = data.readInt(); //头像框
                    var arg7 = data.readUTF(); //玩家名字
                    var t_player = VoLogPlayerKfwz.getFromPool();
    ...
    /** 清空日志 */
    ModelKfwz.prototype.clearLogList = function () {
        var t = this;
        for (var i = t.logVoList.length - 1; i >= 0; i--) {
            var t_vo = t.logVoList[i];
            VoLogKfwz.recycleToPool(t_vo);
            t.logVoList.splice(i, 1);
        }
    };
    /**10854 [I-U-B] GC 本区间队伍信息 [I:队伍idU:队长名字B:参与人物数量]teams*/
    ModelKfwz.prototype.GC_CrossTeamKing_teaminfos_10854 = function (self, data) {
        self.clearTeamList();
        var len = data.readShort();
        for (var i = 0; i < len; i++) {
            var arg1 = data.readInt(); //队伍id
            var arg2 = data.readInt(); //头像
            var arg3 = data.readInt(); //头像框
            var arg4 = data.readUTF(); //队长名字
            var arg5 = data.readByte(); //队伍人数
            var t_vo = VoTeamListKfwz.getFromPool();
            t_vo.teamId = arg1;
            t_vo.head = arg2;
            t_vo.headGrid = arg3;
            t_vo.name = arg4;
    ...
    /** 清空队伍列表 */
    ModelKfwz.prototype.clearTeamList = function () {
        var t = this;
        for (var i = t.teamList.length - 1; i >= 0; i--) {
            var t_vo = t.teamList[i];
            VoTeamListKfwz.recycleToPool(t_vo);
            t.teamList.splice(i, 1);
        }
    };
    ModelKfwz.prototype.cmdSendOverPVE = function (pResult) {
        var t = this;
        var t_killCount = 0;
        for (var _i = 0, _a = t.enemyBattleList; _i < _a.length; _i++) {
            var v = _a[_i];
            if (v.isDead) {
                t_killCount++;
            }
    ...
    ModelKfwz.prototype.CG_CrossTeamKing_overPveBattle_10857 = function (arg1, arg2) {
        var t = this;
        if (t.overWaiting)
            return;
        var bates = this.getBytes();
    ...
    ModelKfwz.prototype.GC_CrossTeamKing_overPveBattle_10858 = function (self, data) {
        var arg1 = data.readByte();
     ...
     */
    ModelKfwz.prototype.showResultPanel = function (pResult) {
        var t = this;
        t.hasResult = true;
        var t_rangeId = t.getRangeId();
        var t_rewardVo = t.getBattleRewardVoByRangeId(t_rangeId);
        if (pResult == 0) {
            //胜利
    ...
    ModelKfwz.prototype.onResultExit = function () {
    ...
    /**10859  CG 进入活动 */
    ModelKfwz.prototype.CG_CrossTeamKing_joinAct_10859 = function () {
        var bates = this.getBytes();
        this.sendSocket(10859, bates);
    };
    /**10860  GC 进入活动成功 */
    ModelKfwz.prototype.GC_CrossTeamKing_joinAct_10860 = function (self, data) {
        self.isInAct = true;
    ...
    ModelKfwz.prototype.CG_CrossTeamKing_buyCount_10861 = function (arg1) {
        var bates = this.getBytes();
        bates.writeInt(arg1);
        this.sendSocket(10861, bates);
    };
    /**10862 B-I-I  B:结果0成功，其他失败resI:已购买次数buyCountI:剩余挑战次数remain*/
    ModelKfwz.prototype.GC_CrossTeamKing_buyCount_10862 = function (self, data) {
        var t_change = false;
        var arg1 = data.readByte();
        var arg2 = data.readInt();
    ...
    ModelKfwz.prototype.CG_CrossTeamKing_quitBattle_10863 = function () {
        var bates = this.getBytes();
        this.sendSocket(10863, bates, true);
    };
    /**10864 I GC连胜数量变化 I:当前连胜数continuitywin*/
    ModelKfwz.prototype.GC_CrossTeamKing_continuitywin_10864 = function (self, data) {
        var t_change = false;
        var arg1 = data.readInt();
        if (arg1 != self.combo) {
            self.combo = arg1;
    ...
    Object.defineProperty(ModelKfwz.prototype, "openTsList", {
        /** 活动开启时间 */
        get: function () {
            var t = this;
            if (t._openTsList === undefined) {
                var t_str = FastAPI.getSystemValue(7924);
                var t_list = JSON.parse(t_str);
    ...
    Object.defineProperty(ModelKfwz.prototype, "isInTeam", {
        /** 是否在队伍中 */
        get: function () {
            var t = this;
    ...
    Object.defineProperty(ModelKfwz.prototype, "areYouLeader", {
        /** 你是否队长 */
        get: function () {
            var t = this;
    ...
    /** 获取转生区间id */
    ModelKfwz.prototype.getRangeId = function () {
        var t = this;
     ...
     * @param pId
     */
    ModelKfwz.prototype.getTargetVoById = function (pId) {
        return this._targetVoMap[pId];
    };
    /** 获取目标数据列表 */
    ModelKfwz.prototype.getTargetVoList = function () {
        var t = this;
        var t_rangeId = t.getRangeId();
        var t_list = t._targetVoListMap[t_rangeId];
        if (t_list === undefined) {
            t._targetVoListMap[t_rangeId] = t_list = [];
            var t_cfg = Config.kfwzmb_770;
            for (var k in t_cfg) {
                var t_vo = t.getTargetVoById(~~k);
    ...
    Object.defineProperty(ModelKfwz.prototype, "maxTargetCount", {
        /** 最大的目标数 */
        get: function () {
            var t = this;
            var t_list = t.getTargetVoList();
            if (t_list.length > 0) {
    ...
    /** 获取段位列表 */
    ModelKfwz.prototype.getGradeList = function () {
        var t = this;
        var t_rangeId = t.getRangeId();
        var t_list = t._gradeListMap[t_rangeId];
        if (t_list === undefined) {
            t._gradeListMap[t_rangeId] = t_list = [];
            var t_cfg = Config.kfwzdw_770;
            for (var k in t_cfg) {
                var t_vo = new VoGradeKfwz();
                t_vo.id = ~~k;
                if (t_vo.cfg.zs == t_rangeId) {
                    t_list.push(t_vo);
                }
     ...
     * @param pGrade 段位
     */
    ModelKfwz.prototype.getGradeVoByGrade = function (pGrade) {
        var t = this;
        var t_list = t.getGradeList();
        return t_list[pGrade - 1];
    };
    /** 获取排行配置列表 */
    ModelKfwz.prototype.getRankCfgListByRangeId = function (pRangeId) {
        var t = this;
        var t_rangeId = pRangeId;
        if (!t._rankCfgListMap) {
            t._rankCfgListMap = {};
            var t_cfg = Config.kfwzph_770;
            for (var k in t_cfg) {
                var t_vo = null;
                var t_obj = t_cfg[k];
                var t_volist = t._rankCfgListMap[t_obj.zs];
                if (t_volist === undefined) {
                    t._rankCfgListMap[t_obj.zs] = t_volist = [];
                }
                var t_pmList = JSON.parse(t_obj.pm);
                var t_min = ~~t_pmList[0][0];
                var t_max = ~~t_pmList[0][1];
                if (t_min == t_max) {
                    t_vo = new VoRankCfgKfwz();
                    t_vo.id = ~~k;
                    t_vo.rank = t_min;
        ...
        var t_list = t._rankCfgListMap[t_rangeId];
        if (t_list === undefined) {
            t_list = [];
        }
        return t_list;
     ...
     * @param pRangeId
     */
    ModelKfwz.prototype.getBattleRewardVoByRangeId = function (pRangeId) {
        var t = this;
        var t_vo = t._battleRewardVoMap[pRangeId];
        if (t_vo === undefined) {
            var t_cfg = Config.kfwztz_770;
            for (var k in t_cfg) {
                var t_tempVo = new VoBattleRewardKfwz();
                t_tempVo.id = ~~k;
    ...
    Object.defineProperty(ModelKfwz.prototype, "vipBuyLimit", {
        /** vip可购买上限 */
        get: function () {
            var t_vipId = Model_player.voMine.viplv + 1;
            var t_cfg = Config.VIP_710[t_vipId];
            if (t_cfg) {
                return t_cfg.kfwz;
            }
            else
    ...
    Object.defineProperty(ModelKfwz.prototype, "vipBuyNeedConsume", {
        /** 购买次数所需消耗 */
        get: function () {
            var t = this;
            if (t._buyCountConsume === undefined) {
                var t_list = ConfigHelp.makeItemListArr(FastAPI.getSystemValue(7923));
     ...
     * @param pCount
     */
    ModelKfwz.prototype.getBuyCountNeedByCount = function (pCount) {
        var t = this;
        if (t._buyCountNeedList === undefined) {
            t._buyCountNeedList = [];
            var t_cfg = Config.kfwzcs_770;
            for (var k in t_cfg) {
                var t_list = ConfigHelp.makeItemListArr(t_cfg[k].xh);
     ...
     * @param pRoleId
     */
    ModelKfwz.prototype.getBattleVoByRoleId = function (pRoleId) {
        var t = this;
     ...
     */
    ModelKfwz.prototype.checkIsAllOver = function () {
        var t = this;
        {
            var t_total = t.enemyBattleList.length;
            var t_deadCount = 0;
            for (var _i = 0, _a = t.enemyBattleList; _i < _a.length; _i++) {
                var v = _a[_i];
                if (v.isDead) {
        ...
        {
            var t_total = t.myBattleList.length;
            var t_deadCount = 0;
            for (var _b = 0, _c = t.myBattleList; _b < _c.length; _b++) {
                var v = _c[_b];
                if (v.isDead) {
                    t_deadCount++;
                }
    ...
    /** 设置自动开始标识 */
    ModelKfwz.prototype.setAutoStartFlag = function (value) {
        var t = this;
    ...
    /** 自动开始倒计时 */
    ModelKfwz.prototype.startAutoEnterTimer = function (pFlag) {
        var t = this;
        t.teamVo.teamId;
        if (pFlag
            && t.isInAct
    ...
    ModelKfwz.prototype.onAutoEnterTimer = function () {
        var t = this;
        t.CG_CrossTeamKing_marryBattle_10839();
    };
    //===================================== private method =====================================
    ModelKfwz.prototype.reddotCheckBox = function () {
        var t = this;
        {
            var t_value = 0;
            var t_boxList = t.getTargetVoList();
...
__reflect(ModelKfwz.prototype, "ModelKfwz");

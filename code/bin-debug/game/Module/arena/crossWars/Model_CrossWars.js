...
var Model_CrossWars = (function (_super) {
    __extends(Model_CrossWars, _super);
    function Model_CrossWars() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    /**2101 CG 打开ui B:比赛进程（0-4）第几轮 */
    Model_CrossWars.prototype.CG_OPENUI = function (type) {
        var bates = this.getBytes();
        bates.writeByte(type);
        this.sendSocket(2101, bates);
    };
    /**2103 CG 买比赛输赢 B:第几轮B:第几场B:1为ID1赢, 2为ID2赢 */
    Model_CrossWars.prototype.CG_BUY_WIN = function (turn, index, sides) {
        var bates = this.getBytes();
    ...
    Model_CrossWars.prototype.CG_LOOK_BATTLE = function (ply) {
        var bates = this.getBytes();
    ...
    /**2109 CG 打开名人堂 */
    Model_CrossWars.prototype.CG_OPEN_WINERS = function () {
        var bates = this.getBytes();
        this.sendSocket(2109, bates);
    };
    /**2111 CG 膜拜 */
    Model_CrossWars.prototype.CG_MOBAI = function () {
        var bates = this.getBytes();
        this.sendSocket(2111, bates);
    };
    /**2113 CG 获取冠军奖励 */
    Model_CrossWars.prototype.CG_GET_FRIST = function () {
        var bates = this.getBytes();
        this.sendSocket(2113, bates);
    };
    //协议处理
    Model_CrossWars.prototype.listenServ = function (mgr) {
        this.socket = mgr;
    ...
    Model_CrossWars.prototype.GC_NOTICE = function (self, data) {
    ...
    Model_CrossWars.prototype.GC_BATTLE_INFOS = function (self, data) {
    ...
    Model_CrossWars.prototype.GC_BUY_WIN = function (self, data) {
    ...
    Model_CrossWars.prototype.GC_LOOK_BATTLE = function (self, data) {
        var len = data.readShort();
        for (var i = 0; i < len; i++) {
            ...
            var ply = Model_CrossWars.matchPlyObj[batPly.id];
            if (ply) {
        ...
        //找战斗结果
        for (var i = 0; i < Model_CrossWars.matchPlyArr.length; i++) {
            var plyArr = Model_CrossWars.matchPlyArr[i];
            var size = plyArr ? plyArr.length : 0;
            for (var j = 0; j < size; j++) {
                var macthPly = plyArr[j];
    ...
    Model_CrossWars.prototype.GC_OPEN_WINERS = function (self, data) {
        var len = data.readShort();
    ...
    Model_CrossWars.prototype.GC_MOBAI = function (self, data) {
        var result = data.readByte();
        if (result == 0) {
            Model_CrossWars.winMobai = data.readByte();
            GGlobal.control.notify(Enum_MsgType.CROSSWARS_OPEN_WINERS);
    ...
    Model_CrossWars.prototype.GC_GET_FRIST = function (self, data) {
        var result = data.readByte();
        if (result == 0) {
            Model_CrossWars.winReward = data.readByte();
            GGlobal.control.notify(Enum_MsgType.CROSSWARS_OPEN_WINERS);
    ...
    Model_CrossWars.getRewardArr = function ($zs) {
        if (Model_CrossWars._rewardArr == null) {
            Model_CrossWars._rewardArr = [];
            for (var keys in Config.xxzb_233) {
                var id = Number(keys);
                var zs = Math.floor(id / 10) - 1;
                var rew = id % 10 - 1;
    ...
    // 1十六强2八强3四强4半决赛5决赛
    Model_CrossWars.getTurnName = function (v) {
        if (v == 1) {
            return "16强赛";
        }
        else if (v == 2) {
    ...
    // 1十六强2八强3四强4半决赛5决赛
    Model_CrossWars.getGroupName = function (v) {
        if (v == 1) {
            return "A组";
        }
        else if (v == 2) {
    ...
    Model_CrossWars.checkNotice = function () {
    ...
    Model_CrossWars.actStatus = 0; //活动状态
    Model_CrossWars.actTurn = 0; //进场进度
    Model_CrossWars.actPeriod = 0; // 1：准备阶段 2：战斗阶段
    Model_CrossWars.matchPlyArr = [];
    Model_CrossWars.matchPlyObj = {};
    Model_CrossWars.battleTurn = 0;
    Model_CrossWars.winMobai = 1; //名人堂膜拜
    Model_CrossWars.winReward = 0; ////名人堂 奖励领取
    Model_CrossWars.winPlyArr = []; //名人堂
    Model_CrossWars.hasData = false; //开打过一次界面有红点数据
    return Model_CrossWars;
}(BaseModel));
__reflect(Model_CrossWars.prototype, "Model_CrossWars");

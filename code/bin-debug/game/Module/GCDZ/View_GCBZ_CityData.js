...
var View_GCBZ_CityData = (function (_super) {
    __extends(View_GCBZ_CityData, _super);
    function View_GCBZ_CityData() {
        var _this = _super.call(this) || this;
        _this.childrenCreated();
        return _this;
    }
    View_GCBZ_CityData.prototype.childrenCreated = function () {
        var self = this;
    ...
    View_GCBZ_CityData.prototype.updateShow = function () {
        var self = this;
        var model = GGlobal.modelgcbz;
        var vo = self.vo;
        self.frame.text = vo.cfg.mz + "驻守大将";
        var itemVo = ConfigHelp.makeItemListArr(JSON.parse(vo.cfg.zsjl))[0];
        var times = Math.ceil(Model_GlobalMsg.getServerTime() / 1000) - vo.times;
        ...
        if (vo.id > 0) {
            self.dataLb.text = ConfigHelp.reTxt("{0}\n战力：{1}\n已驻守：{2}\n共收益：{3}", vo.id == Model_player.voMine.id ? HtmlUtil.fontNoSize(vo.name, Color.getColorStr(2)) : vo.name, vo.power, DateUtil2.formatUsedTime(times, "hh时uu分ss秒"), Math.floor(times / (ConfigHelp.getSystemNum(8250) * 60)) * itemVo.count + itemVo.name);
    ...
    View_GCBZ_CityData.prototype.onShown = function () {
        var self = this;
        self.vo = self._args;
        self.updateShow();
    ...
    View_GCBZ_CityData.prototype.OnZS = function () {
        var self = this;
        var model = GGlobal.modelgcbz;
    ...
    // 	}
    // }
    View_GCBZ_CityData.prototype.onHide = function () {
        var self = this;
        self.zsBt.removeClickListener(self.OnZS, self);
    };
    View_GCBZ_CityData.URL = "ui://vgiijkm8r5gem";
    return View_GCBZ_CityData;
}(UIModalPanel));
__reflect(View_GCBZ_CityData.prototype, "View_GCBZ_CityData");

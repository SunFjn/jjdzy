...
var Model_LingLong = (function (_super) {
    __extends(Model_LingLong, _super);
    function Model_LingLong() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    /**2221 打开界面 */
    Model_LingLong.prototype.CG_OPENUI = function () {
        var bates = this.getBytes();
        this.sendSocket(2221, bates);
    };
    /**2223 购买 B:购买次数，1次或10次B:购买类型，0：玲珑币购买，1：元宝购买 */
    Model_LingLong.prototype.CG_BUY = function (count, type) {
        var bates = this.getBytes();
    ...
    Model_LingLong.prototype.CG_RANKUI = function (type) {
        var bates = this.getBytes();
        bates.writeByte(type);
        this.sendSocket(2225, bates);
    };
    /**2227 领取每日积分宝箱奖励 I:玲珑阁积分表id */
    Model_LingLong.prototype.CG_GET_SCORE_AWARD = function (id) {
        var bates = this.getBytes();
        bates.writeInt(id);
        this.sendSocket(2227, bates);
    };
    /**打开上期排名界面 */
    Model_LingLong.prototype.CG_OPEN_LAST_RANK = function () {
        var bates = this.getBytes();
        this.sendSocket(2233, bates);
    };
    /**打开上期排名界面 */
    Model_LingLong.prototype.CG_OPEN_LAST_ZONE = function () {
        var bates = this.getBytes();
        this.sendSocket(2235, bates);
    };
    //协议处理
    Model_LingLong.prototype.listenServ = function (mgr) {
        this.socket = mgr;
    ...
    Model_LingLong.prototype.GC_BUY = function (self, data) {
        ...
        if (result == 1) {
            ...
            var dTime = Model_LingLong.skipTween ? 0 : 1200;
            setTimeout(function () {
    ...
    Model_LingLong.prototype.GC_RANKUI = function (self, data) {
        var len = data.readShort();
        Model_LingLong.rankArr = [];
        for (var i = 0; i < len; i++) {
            var vo = new Vo_LingLong();
            vo.readMsgRank(data);
            vo.rank = i + 1;
    ...
    Model_LingLong.prototype.GC_RANK1UI = function (self, data) {
        var len = data.readShort();
        ...
        var voSelf = new Vo_LingLong();
        voSelf.status = 1;
        voSelf.rank = data.readInt();
        voSelf.point = data.readInt();
        var hasSelf = false;
        for (var i = 0; i < len; i++) {
            var vo = Model_LingLong.rank1Arr[i];
    ...
    Model_LingLong.prototype.GC_LOGS = function (self, data) {
        var len = data.readShort();
        for (var i = 0; i < len; i++) {
            var vLog = new Vo_LingLong();
            vLog.readMsgLog(data);
    ...
    Model_LingLong.prototype.GC_OPEN_LAST_RANK = function (self, data) {
        var len = data.readShort();
    ...
    Model_LingLong.prototype.GC_OPEN_LAST_ZONE = function (self, data) {
        var len = data.readShort();
        ...
        var hasSelf = false;
        for (var i = 0; i < len; i++) {
            var vo = Model_LingLong.rankLast1Arr[i];
            if (vo.rank == voSelf.rank) {
                vo.status = 1;
                hasSelf = true;
                break;
    ...
    Model_LingLong.getLLgCfg = function (llg, rank) {
        if (Model_LingLong._llgCfg == null) {
            Model_LingLong._llgCfg = {};
            for (var keys in Config.llgrank_239) {
                var llCfg = Config.llgrank_239[keys];
                if (Model_LingLong._llgCfg[llCfg.llg] == null) {
                    Model_LingLong._llgCfg[llCfg.llg] = {};
                }
                var arr = ConfigHelp.SplitStr(llCfg.rank);
    ...
    Model_LingLong.getLLGRank1 = function (llg, rank) {
        if (Model_LingLong._llgRank1 == null) {
            Model_LingLong._llgRank1 = {};
            for (var keys in Config.llgqf_239) {
                var llCfg = Config.llgqf_239[keys];
    ...
    Model_LingLong.cfgId = 0;
    Model_LingLong.lastCount = -1;
    Model_LingLong.logsArr = [];
    Model_LingLong.lingLong = 0; //积分表
    Model_LingLong.pointArr = [];
    Model_LingLong.myPoint = 0;
    Model_LingLong.rankArr = [];
    Model_LingLong.rank1Arr = [];
    Model_LingLong.rankId = 0;
    Model_LingLong.rankLastArr = [];
    Model_LingLong.rankLastId = 0;
    Model_LingLong.myLastPoint = 0;
    Model_LingLong.rankLast1Arr = [];
    Model_LingLong.rankLast1Id = 0;
    Model_LingLong.lingLongId = 410023; //玲珑币
    Model_LingLong.skipTween = false; //是否跳过动画
    return Model_LingLong;
}(BaseModel));
__reflect(Model_LingLong.prototype, "Model_LingLong");

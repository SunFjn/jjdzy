...
var Child_TianShu_JiBan = (function (_super) {
    __extends(Child_TianShu_JiBan, _super);
    function Child_TianShu_JiBan() {
    ...
    Child_TianShu_JiBan.createInstance = function () {
        return (fairygui.UIPackage.createObject("role", "Child_BaoWu_JiBan"));
    };
    Child_TianShu_JiBan.prototype.constructFromXML = function (xml) {
    ...
    Child_TianShu_JiBan.prototype.open = function () {
        var s = this;
        s.eventFunction(1);
        GGlobal.control.listen(Enum_MsgType.BY_SYS_JI_BAN, s.jiBanUp, s);
        s.update();
    };
    Child_TianShu_JiBan.prototype.close = function () {
        var s = this;
        s.eventFunction(0);
    ...
    Child_TianShu_JiBan.prototype.jiBanUp = function (arr) {
        var sf = this;
        var sys = arr[0];
        var index = arr[1];
        var sid = arr[2];
        if (sys != Model_BySys.JIB_TIANSHU) {
            return;
        }
        sf._jiBanArr = Model_BySys.getJiBan(Model_BySys.JIB_TIANSHU);
    ...
    Child_TianShu_JiBan.prototype.update = function () {
        var sf = this;
        sf._jiBanArr = Model_BySys.getJiBan(Model_BySys.JIB_TIANSHU);
        sf.list.numItems = sf._jiBanArr.length;
    ...
    Child_TianShu_JiBan.prototype.upPower = function () {
        //套装战力
        var s = this;
        var power = 0;
        if (s._jiBanArr) {
            for (var i = 0; i < s._jiBanArr.length; i++) {
                ...
                var suit = Config.booksuit_215[ids];
                if (suit) {
    ...
    Child_TianShu_JiBan.prototype.renderHander = function (index, obj) {
        var s = this;
        var gird = obj;
        var suitId = s._jiBanArr[index];
        var suit = Config.booksuit_215[suitId];
        gird.title = suit.name;
        gird.data = suitId;
        gird.checkNotice = Model_BySys.checkSuitVo(suitId, Model_BySys.JIB_TIANSHU);
    };
    Child_TianShu_JiBan.prototype.renderGrid = function (index, obj) {
        var sf = this;
        var v = obj;
        v.setSuitVo(sf._conditionArr[index]);
    };
    Child_TianShu_JiBan.prototype.onClickList = function (e) {
        var sf = this;
        var selectItem = e.itemObject;
        sf.selectdUpdate(selectItem.data);
    };
    Child_TianShu_JiBan.prototype.selectdUpdate = function (suitId) {
        var sf = this;
        if (suitId == null) {
            return;
        }
        ...
        var suit = Config.booksuit_215[suitId];
        var suitNext = Config.booksuit_215[suitId + 1];
        if (level == 0) {
        ...
        if (suit.condition == "0") {
            ...
            var suitPre = Config.booksuit_215[suitId - 1];
            ...
            sf.btnActivity.enabled = sf.btnActivity.checkNotice = Model_BySys.checkSuitVo(suit.suitid, Model_BySys.JIB_TIANSHU);
            var allNum = 0;
            var reachNum = 0;
            var need;
        ...
        IconUtil.setImg(sf.imgName, Enum_Path.JIBAN_URL + "n" + UIConst.TIANSHU + ".png");
        IconUtil.setImg(sf.imgIcon, Enum_Path.JIBAN_URL + "i" + UIConst.TIANSHU + ".png");
    };
    Child_TianShu_JiBan.prototype.onClickActivity = function () {
        var sf = this;
        if (!sf._hasCondition) {
        ...
        GGlobal.modelBySys.CGJiBanUp(Model_BySys.JIB_TIANSHU, index);
    };
    Child_TianShu_JiBan.prototype.pageHandler = function (event) {
        var sf = this;
    ...
    Child_TianShu_JiBan.prototype.onExplain = function (e) {
        GGlobal.layerMgr.open(UIConst.WFLAB_PANEL);
        e.stopPropagation();
        e.stopImmediatePropagation();
    };
    Child_TianShu_JiBan.prototype.jcTxt = function (jc, color) {
        var sf = this;
        if (color)
    ...
    Child_TianShu_JiBan.URL = "ui://3tzqotadby8m38";
    return Child_TianShu_JiBan;
}(fairygui.GComponent));
__reflect(Child_TianShu_JiBan.prototype, "Child_TianShu_JiBan");

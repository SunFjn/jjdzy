class KingShipCtrl extends SceneCtrl {
	public constructor() {
		super();
	}

	private static _instance: KingShipCtrl;
	public static get instance(): KingShipCtrl {
		if (!KingShipCtrl._instance) KingShipCtrl._instance = new KingShipCtrl();
	...
	public onExit(scene: MapScene): void {
		...
		if (GGlobal.layerMgr.lastPanelId <= 0) GGlobal.layerMgr.open(UIConst.COUNTRY_KINGSHIP);
	}
	...
	public setState(st: number): void {
		if (st == 0) {
			let kingVo = Model_Kingship.guardArr[Model_Kingship.battlePos - 1];
			if (kingVo.sid == 0) {
				this.enemy = this.createEnemys(kingVo.id);
			} else {
				let vo = GGlobal.modelPlayer.playerDetailDic[kingVo.sid];
				this.addOther(vo);

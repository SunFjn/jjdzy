class HscbSceneCtrl extends SceneCtrl {

	public constructor() {
		super();
	}

	private static _inst: HscbSceneCtrl;
	...
	onEnter(scene: MapScene) {
		...
		var cfg: Ihscb_751 = this.getCurBoss();
		var bossId = Number(JSON.parse(cfg.boss)[0][0]);
		let batLayer = GGlobal.model_HSCB.batLayer
		let bigTower = GGlobal.model_HSCB.getBigRewCfg(batLayer);
		if (bigTower && batLayer <= bigTower.id) {//大奖
			let rewardVo: IGridImpl = ConfigHelp.makeItemListArr(JSON.parse(bigTower.dj))[0];
	...
	protected trytoOpenPnl() {
		if (GGlobal.layerMgr.lastPanelId <= 0) GGlobal.layerMgr.open(UIConst.CHILD_HSCB)
	}

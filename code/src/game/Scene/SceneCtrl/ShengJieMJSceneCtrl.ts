class ShengJieMJSceneCtrl extends BossCtrl {
	public constructor() {
		super();
	}

	private static _inst: ShengJieMJSceneCtrl;
	...
	public onEnter(scene: MapScene) {
		...
		s.createEnemys();
		let id = ItemTeam.curMiJing.data.id;
		const mapid = Config.sjmj_258[id / 1000 >> 0].map;
		s.setMapHead(mapid);
		s.setSt(0);
	...
	protected updateData(): void {
		...
		if (!(s.initState & 1)) {//是否初始化
			...
			const id = ItemTeam.curMiJing.data.id;
			const cfg = Config.sjmjfb_258[id];
		...
		if (ModelShengJieMJ.bossHP <= 0) {//BOSS被击杀
			s.setSt(2);
		}

		for (let j = 0; j < s.roleList.length; j++) {
			for (let i = 0; i < ModelShengJieMJ.hpArr.length; i++) {
	...
	public createMyChars() {
		...
		let teamArr = GGlobal.modelSJMJ.teamInfos;

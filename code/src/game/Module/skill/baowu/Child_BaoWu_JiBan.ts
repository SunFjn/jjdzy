class Child_BaoWu_JiBan extends fairygui.GComponent {
	...
	private _conditionArr: Array<Vo_BaoWu>;
	private sysName = "宝物"

	public static createInstance(): Child_BaoWu_JiBan {
		return <Child_BaoWu_JiBan><any>(fairygui.UIPackage.createObject("role", "Child_BaoWu_JiBan"));
	}
	...
	private upPower(): void {
		...
		if (s._jiBanArr) {
			for (let i = 0; i < s._jiBanArr.length; i++) {
				...
				let suit = Config.baosuit_214[ids];
				if (suit) {
	...
	private renderGrid(index: number, obj: fairygui.GObject): void {
		const sf = this;
		let v: ViewBWGrid = obj as ViewBWGrid

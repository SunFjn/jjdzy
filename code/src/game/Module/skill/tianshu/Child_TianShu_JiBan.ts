class Child_TianShu_JiBan extends fairygui.GComponent {
	...
	private _conditionArr: Array<VoTianShu>;
	private sysName = "天书"

	public static createInstance(): Child_TianShu_JiBan {
		return <Child_TianShu_JiBan><any>(fairygui.UIPackage.createObject("role", "Child_BaoWu_JiBan"));
	}
	...
	private upPower(): void {
		...
		if (s._jiBanArr) {
			for (let i = 0; i < s._jiBanArr.length; i++) {
				...
				let suit = Config.booksuit_215[ids];
				if (suit) {
	...
	private renderGrid(index: number, obj: fairygui.GObject): void {
		const sf = this;
		let v: TianShuItem = obj as TianShuItem

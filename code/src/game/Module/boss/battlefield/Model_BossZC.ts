class Model_BossZC extends BaseModel {
	public constructor() {
		super();
	}
	static CROSS = 2;
	 ...
	 * B:状态（0：副本初始状态 被击杀后进入倒计时，1准备可以进入，2玩家互相pk时间 不可进入 3玩家开始抢夺boss，击杀boss，不可以进入副本 4击杀boss状态（规定时间未击杀完成boss），可以进入副本）]
	*/
	public GCopenUI(m: Model_BossZC, b: BaseBytes) {
		let type = b.readByte();
		m.sceneType = type;
		let len = b.readShort();
		let temp = [];
		...
		let systemID = type == 1 ? UIConst.BOSS_BATTLEFIELD_LOCAL : UIConst.BOSS_BATTLEFIELD_CROSS;
		let redDot = false;
		for (let i = 0; i < len; i++) {
			let id = b.readInt();
			let time = b.readLong() * 1000 + now;
			let state = b.readByte();
			let cd = b.readInt() * 1000 + now;
			let killer = b.readUTF();
			let isFirst = b.readByte() == 0;
			let cfg = Config.bosszc_010[id];
			let quality = cfg.pinzhi;
			let condition = JSON.parse(cfg.tiaojian)[0];
			if (zs >= condition[0] && zs <= condition[1]) {//不是自己当前的不显示
				let weight = quality * 10000;
	...
	*/
	public GCenterWarScene(m: Model_BossZC, b: BaseBytes) {
		let ret = b.readByte();
		let idx = b.readInt();
		let cd = b.readInt();
		if (ret == 0) {
	...
	*/
	public GCfubenStUpdate(m: Model_BossZC, b: BaseBytes) {
		let idx = b.readInt();
		let st = b.readByte();
		let remaindTime = b.readInt();
		let now = Model_GlobalMsg.getServerTime();
		if (idx == m.sceneId) {
	...
	*/
	private GCfight(m: Model_BossZC, b: BaseBytes) {
		let st = this.sceneState;
		if (st != 2) {
			if (st == 3) {
	...
	*/
	private GCfightBoss4460(m: Model_BossZC, b: BaseBytes) {
		let ret = b.readByte();
		if (ret == 0) {
	...
	*/
	public GCExite(m: Model_BossZC, b: BaseBytes) {
		BossZCManager.exite();
	}

	//GC 挑战结果 B:0成功挑战 1对方正在战斗 
	private GCfightPlayer4466(m: Model_BossZC, b: BaseBytes) {
		let ret = b.readByte();
		if (ret != 0) {
	...
	private GCBossBattleEnd(m: Model_BossZC, ba: BaseBytes) {
		let ret = ba.readByte();
		var awards = [];
		for (var i = 0, len = ba.readShort(); i < len; i++) {
			let vo = ConfigHelp.parseItemBa(ba);
	...
	private GCSCENEPlayerSt(m: Model_BossZC, b: BaseBytes) {
		let len = b.readShort();
		for (let i = 0; i < len; i++) {
			let type = b.readByte();
			let st = b.readByte();
			let id = b.readLong();
			let role = GameUnitManager.findUnitByID(id);
			if (role) {
				if (st == 1) {
					let plug = RoleStatePlug.create(role);
					plug.setState(1);
	...
	private GCBOSSDEAD(m: Model_BossZC, b: BaseBytes) {

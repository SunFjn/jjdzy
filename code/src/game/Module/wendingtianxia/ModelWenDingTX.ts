/**问鼎天下*/class ModelWenDingTX extends BaseModel {
	public constructor() {
		super();
	}


	/**活动结束时间*/activiyEndTime = 0;
	/**收益时间*/fixedTimeAwards = 0;
	...
	/**当前玉玺持有者*/mvp_name = "";
	/**当前玉玺持有者id*/mvp_id = 0;
	/**当前玉玺持有者头像id*/mvpHead_id = 0;
	/**当前玉玺持有者头像框id*/mvpHeadGrid_id = 0;
	/**连斩次数*/kill_count = 0;
	/**杀人书*/kill = 0;
	/**BUFF层数*/buff = 0;
	...
	/**获取当前层的奖励*/
	getNowLayerAwardsStr(layer) {
		let cfg = Config.wdtx_260[layer];
		let str = ConfigHelp.makeItemRewardText(cfg.reward);
		return str;
	}

	//活动是否跨服
	getActivityIsCross() {
		let cfgday = ConfigHelp.getSystemNum(4705);
		let sday = Model_GlobalMsg.kaifuDay;
		return cfgday > sday;
	}
	...
	decodeRankCfg() {
		let cfg = Config.wdtxrank_260;
		let isCross = this.getActivityIsCross();
		this.rankCfg = {};
		for (let i in cfg) {
			if (isCross) {
				if (cfg[i].qf == 1) {
	...
	getRankItemCFG(rank) {
		let ret;
		let cfgs = this.getRankCfg();
		for (let i in cfgs) {
			let rankinfo = JSON.parse(cfgs[i].rank);
			let startRank = rankinfo[0][0];
			let endRank = rankinfo[0][1];
	...
	private updatePlayer(id) {
		...
		if (role) {
			...
			let name: WenDingTXNamePlug = WenDingTXNamePlug.create();
			name.role = role;
			role.addPlug(name);
		}
	}
	...
	private checkWDTXZhandi() {
		let ret = false;
		let arr = this.rank_LianZhan;
		for (let i = 0; i < arr.length; i++) {
			if (arr[i][1] == 1) {
				ret = true;
				break;
	...
	public sortAndsetNoticeScoreRank() {
		let m = this;
		m.rank_Score = m.rank_Score.sort(function (a, b) {
			return a[2] < b[2] ? -1 : 1;
		});
		let ret = false;
		let len = m.rank_Score.length;
		for (let i = 0; i < len; i++) {
			let st = m.rank_Score[i][1];
			if (st == 1) {
				ret = true;
				break;
			}
	...
	private _checkOver_Layer;
	private _checkOver_score;
	//前端检查是否有----积分奖励可领取
	public updateScoreRank() {
		if (this._checkOver_score) return;//领完全不
		let ret = GGlobal.reddot.checkCondition(UIConst.WENDINGTX, 3);
		if (ret) return;//已经有红点
		let dta = this.rank_Score;
		if (!dta || !dta.length) return;//尚无数据
		let now = this.score;
		let len = dta.length;
		let completeCount = 0;
		for (let i = 0; i < len; i++) {
			...
			let st = item[1];
			if (st == 0) {
				let cfg = Config.wdtxpoint_260[id];
				let nextScore = cfg.point;
				if (now >= nextScore) {
	...
	//前端检查是否有----楼层奖励奖励可领取
	public updateLayerRank() {
		if (this._checkOver_Layer) return;//领完全不
		let ret = GGlobal.reddot.checkCondition(UIConst.WENDINGTX, 3);
		if (ret) return;//已经有红点
		let dta = this.rank_Layer;
		if (!dta || !dta.length) return;//尚无数据
		let now = this.layer;
		let len = dta.length;
		let completeCount = 0;
	...
	private addNPCName(id) {
		var npc: ARPGNpc = GameUnitManager.findUnit(id, UnitType.NPC) as ARPGNpc;
		if (npc) {
			let s = this;
			let cfg = npc.vo.cfg;
			let score = Config.wdtx_260[GGlobal.modelWenDingTX.layer].point;
	...
	*/
	private mineUpdate4202(m: ModelWenDingTX, ba: BaseBytes) {
	...
	*/
	private sceneMvpUpdate4204(m: ModelWenDingTX, ba: BaseBytes) {
	...
	*/
	private layerUpdate4206(m: ModelWenDingTX, ba: BaseBytes) {
		let ret = ba.readByte();
		var layer = ba.readByte();
		if (m.layer != layer) {
			m.updateLayerRank();
			if (GameUnitManager.hero) GameUnitManager.hero.showEffect();
		}
		m.layer = layer;
		if (ret == 1) {
	...
	*/
	private fight4208(m: ModelWenDingTX, ba: BaseBytes) {
		let ret = ba.readByte();
		let a = '';
		switch (ret) {
			case 1: break;
			case 2: a = "该玩家不存在 "; break;
			case 3: a = "本玩家问鼎天下数据不存在"; break;
			case 4: a = "该玩家问鼎天下数据不存在"; break;
			case 5: a = "该玩家不在房间"; break;
			case 6: a = "你不在房间中"; break;
			case 7: a = "该玩家已经不在本层"; break;
			case 8: a = "请退出当前场景"; break;
			case 9: a = "对方正在战斗中"; break;
			case 10: a = "你未复活"; break;
			case 11: a = "该玩家未复活"; break;
			case 12: a = "活动未开启"; break;
			case 13: a = "活动已结束"; break;
		}
	...
	*/
	private score4210(m: ModelWenDingTX, ba: BaseBytes) {
		let score = ba.readShort();
		if (score > 0) ViewCommonWarn.text("获得积分 " + score);
		else ViewCommonWarn.text("失去积分 " + score);
	}
	/**
	 * 4212  U-S-S 
	 * 活动结算界面 U:玩家名字S:头像IDS:头像框
	*/
	private activityEnd4212(m: ModelWenDingTX, ba: BaseBytes) {
		m.mvp_name = ba.readUTF();
		m.mvpHead_id = ba.readShort();
		m.mvpHeadGrid_id = ba.readShort();
	 ...
	 * （跨服）打开排名排行榜 
	*/
	public totalRank4213(id = 0, mid = 0) {
		let ba = this.getBytes();
	...
	*/
	private totalRank4214(m: ModelWenDingTX, ba: BaseBytes) {
	...
	*/
	private killCountRank4216(m: ModelWenDingTX, ba: BaseBytes) {
		let len = ba.readShort();
		m.rank_LianZhan = [];
		for (let i = 0; i < len; i++) {
			let id = ba.readByte();
			let st = ba.readByte();
			let weight;
			if (st == 0) weight = id - 100;
			if (st == 1) weight = id - 10000;
			if (st == 2) weight = id;
	...
	*/
	private layerRank4218(m: ModelWenDingTX, ba: BaseBytes) {
		let len = ba.readShort();
		m.rank_Layer = [];
		for (let i = 0; i < len; i++) {
			let id = ba.readByte();
	...
	*/
	 ...
	 * 领取连斩奖励 B:结果 1成功
	*/
	private lianzhanAwards4220(m: ModelWenDingTX, ba: BaseBytes) {
		let ret = ba.readByte();
		if (ret == 1) {
			let len = m.rank_LianZhan.length;
			for (let i = 0; i < len; i++) {
				let data = m.rank_LianZhan[i];
				if (data[0] == m.lid) {
	...
	*/
	 ...
	 *  领取楼层奖励 B:结果 1成功 
	*/
	private layerAwards4222(m: ModelWenDingTX, ba: BaseBytes) {
		let ret = ba.readByte();
		if (ret == 1) {
			let len = m.rank_Layer.length;
			for (let i = 0; i < len; i++) {
				let data = m.rank_Layer[i];
				if (data[0] == m.layerid) {
					m.rank_Layer[i][1] = 2;
	...
	*/
	private enterAct4224(m: ModelWenDingTX, ba: BaseBytes) {
		let ret = ba.readByte();
		let cd = ba.readByte();
		if (ret == 1) {
			ModelArpgMap.getInstance().isServerControlMap = false;
		...
		else if (ret == 2) ViewCommonWarn.text("系统未开启");
		else if (ret == 3) ViewCommonWarn.text("活动未开启");
		else if (ret == 4) ViewCommonWarn.text("请等待" + cd + "秒再进入");
		else if (ret == 5) ViewCommonWarn.text("活动已结束");
	}

	//退出战斗后重新进入场景
	public reApplyEnter4225() {
		let ba = this.getBytes();
	 ...
	 * */
	public sceneUpdate4226(m: ModelWenDingTX, ba: BaseBytes) {
		let id = ba.readLong();
		let info = ba.readUTF();
		...
		let val;
		let plug: WenDingTXNamePlug = role.getPlugBytype(WenDingTXNamePlug);
		for (let i in obj) {
			val = obj[i];
			switch (i) {
				case "jf": plug.setJiFen(val); break;
				case "num": plug.setChaoShen(val); break;
				case "yx": plug.setYuxi(val == 1); break;
				case "wjzt":
					let STplug: WenDingTXStatePlug = role.getPlugBytype(WenDingTXStatePlug);
					if (val == 0) {
						if (STplug) {
					...
					break;
				case "wjqx": plug.setHp(val); break;
			}
		}
	}
	...
	*/
	...
	private fight4228(m: ModelWenDingTX, ba: BaseBytes) {
		let ret = ba.readByte();
		if (ret == 1) {
			let scenectrl: WenDingTXPVECtrl = SceneCtrl.getCtrl(SceneCtrl.WDTX_PVE) as WenDingTXPVECtrl;
			GGlobal.mapscene.scenetype = SceneCtrl.WDTX_PVE;
			WenDingTXManager.enterBattle();
			GGlobal.mapscene.enterSceneCtrl(scenectrl);
		} else {

			let strFun = ViewCommonWarn.text;
			switch (ret) {
				case 2: strFun("你不在活动房间内"); break;
				case 3: strFun("玩家数据异常"); break;
				case 4: strFun("该层怪物未初始化"); break;
				case 5: strFun("怪物已被击败"); break;
				case 6: strFun("怪物战斗中"); break;
				case 7: strFun("你已死亡"); break;
				case 8: strFun("请退出当前场景"); break;
			}
		}
	}
	...
	*/
	public deadTime = 0;
	private heroSt4230(m: ModelWenDingTX, ba: BaseBytes) {
		let heroname = ba.readUTF();
		let floor = ba.readByte();
		if (floor == 0) {
		...
		let cfgT = ConfigHelp.getSystemNum(1012);
	...
	*/
	private heroRelife4232(m: ModelWenDingTX, ba: BaseBytes) {
		let ret = ba.readByte();
		let a = '';
		switch (ret) {
			case 1: a = "复活成功";
	...
	*/
	 ...
	 * 挑战怪物结束 B:结果 1成功
	*/
	private fightEnd4234(m: ModelWenDingTX, ba: BaseBytes) {
		let ret = ba.readByte();
		let str = "";
		switch (ret) {
			case 2: str = "不在活动房间内"; break;
			case 3: str = "玩家数据异常"; break;
			case 4: str = "该层怪物未初始化"; break;
			case 5: str = "你没有与怪物战斗"; break;
			case 6: str = "怪物不存在"; break;
		}
		ViewCommonWarn.text(str);
		let battleret = ba.readByte();
		let awards = ConfigHelp.parseItemListBa(ba);
	...
	*/
	private rankScore4236(m: ModelWenDingTX, ba: BaseBytes) {
		let len = ba.readShort();
		m.rank_Score = [];
		for (let i = 0; i < len; i++) {
			let id = ba.readByte();
	...
	*/
	private rankScore4238(m: ModelWenDingTX, ba: BaseBytes) {
		let ret = ba.readByte();
		let id = ba.readByte();
		if (ret == 1) {
			let len = m.rank_Score.length;
			for (let i = 0; i < len; i++) {
				let ids = m.rank_Score[i][0];
				if (id == ids) {
					m.rank_Score[i][1] = 2;
					m.rank_Score[i][2] = id;
					break;

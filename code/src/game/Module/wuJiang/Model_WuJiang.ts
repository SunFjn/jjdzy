class Model_WuJiang extends BaseModel {

	public static jieShu: number = 1;
	public static exp: number;

	public static wuJiangStar: any = {};
	public static wujiangGodPower: any[] = [];
	public static skillArr: number[] = [];
	public static danShuxing: number;
	public static selectJob = 0;
	public static shenjiangzhiliSkillLv: any[] = [];

	private static _PuTongWujiang: Array<Ihero_211>;
	private static _wuJiangArr: Array<Ihero_211>;

	public static getWuJiangStarByJob(job = 0) {
	...
	public static get wuJiangArr(): Array<Ihero_211> {
	...
	private static _newWuJiang: any;
	public static get newWuJiang() {
	...
	public static get PuTongWujiang() {
	...
	public static initWuJiang(): void {
		...
		for (let keys in Config.hero_211) {
			...
			let act = ConfigHelp.SplitStr(v.activation)
	...
	//	获取某种品质的总星级
	static _starMap: any = {};
	public static getQuilityTotalStar(quility) {
	...
	//计算星级
	calculationQuilityStar = () => {
		let config = Config.hero_211;
		let arr = Model_WuJiang.wuJiangStar;
		let star = 0;
		let temp = {};
		for (let i in arr) {
			let cfg = config[i];
	...
	//是否是神将
	static isGodWuJiang(type) {
		return Config.hero_211[type].godhero == 1;
	}

	/**	武将培养丹 */
	public static DAN_LEVELUP = 411001;
	/**	武将培养丹 提升经验*/
	public static DAN_EXP = 10;
	/**	武将属性丹 加属性低*/
	public static DAN_SHUXING = 412001;
	public static DRUG_SHUXING = 1;
	/**	武将资质丹 加属性高*/
	// public static DAN_ZIZHI = 412002;
	// public static DRUG_ZIZHI = 2;
	/**	武将技能书*/
	public static SKILL_BOOK = 414001;
	/**	将印碎片*/
	public static JIANG_YIN = 410006;

	/**官衔 开启*/
	public static _OPEN_GUAN = null//[1, 18, 20, 25, 28, 30, 35, 38, 40, 45];

	//时装信息
	public static shiZhuanDic: { [id: number]: { onSkinId: number, ownSkinIds: { skinId: number, starLv: number }[] } } = <any>{};
	public static itemShiZhuang: { [id: number]: { star: number, maxStar: number } } = {};//道具对应激活的时装数据
	public static curSelWJId: number;//当前选择的武将id
	public static msg_data_shiZhuang = "msg_data_shiZhuang";

	public isMaxLevelSZ(id) {
		let dic = Model_WuJiang.itemShiZhuang;
		if (dic[id]) {
	...
	public static LOCK_ICON = [930170, 930171, 930172, 930173, 930174, 930175, 930176, 930177, 930178, 930179];

	/**651 CG 获取武将信息 */
	public CGGetWuJiang(): void {
		var bates = this.getBytes();
		this.sendSocket(651, bates);
	}

	/**653	 CG 武将升阶 B:升阶方式0普通 1一键 */
	public CGUpWuJie(type): void {
		var bates = this.getBytes();
	...
	public static wujiangzhiliName: string = "";
	public static wujiangzhiliType: number = 0;
	public static wujiangzhiliStar: number = 0;
	//652 GC 武将状态 [I:武将typeI:武将星级I:神将之力星级I:神将之力技能进阶]I:武将阶数I:经验[I:技能等级0表示未激活]I:培养丹数量I:资质丹数量
	private GCWuJiangSate(self: Model_WuJiang, data: BaseBytes): void {
		var len = data.readShort();
		for (let i = 0; i < len; i++) {
			let type = data.readInt();
			let star = data.readInt();
			let godPower = data.readInt();
			let skillLv = data.readInt();
			...
			if (type == Model_player.voMine.job) {
				...
				let godskillCfg: Iherogodskill_211 = Config.herogodskill_211[type * 100 + skillLv];
				for (let j = 0; j < skillList.length; j++) {
					let skillVo = skillList[j];
					let damage = 0;
	...
	/**获取神将之力的红点状态 */
	private getWujiangzhiliNotice(id: number, godPower: number, star: number) {
		if (godPower >= Config.hero_211[id].max) {//等级最大的情况下，直接false
			return false;
		}
		let index = id * 100 + (godPower == 0 ? 1 : godPower);
		let temp;
		if (godPower > 0) {
	...
	private getNextData(heroId: number) {
		let isGet = false;
		let tempData: any = Config.herogod_211
	 ...
	 * 获取神将之力技能进阶的红点状态 
	 * */
	private getWujiangzhiliSkillNotice(id: number, godPower: number, skillLv: number): boolean {
		if (godPower <= 0) {
			return false;
		}
		let cfgId: number = id * 100 + skillLv;
		let curCfg: Iherogodskill_211 = Config.herogodskill_211[cfgId];
		let nextCfg: Iherogodskill_211 = Config.herogodskill_211[cfgId + 1];
		if (nextCfg) {
			let costArr = JSON.parse(nextCfg.consume);
	...
	private GCUpWuJie(self: Model_WuJiang, data: BaseBytes): void {
	...
	private GCJihuoSkill(self: Model_WuJiang, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 1) {
	...
	private GCJihuowj(self: Model_WuJiang, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 1) {
	...
	private GCUpWJStar(self: Model_WuJiang, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 1) {
			var type = data.readByte();
	...
	private GCUseDan(self: Model_WuJiang, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 1) {
			Model_WuJiang.danShuxing = data.readInt();
			let danZizhi = data.readInt();
	...
	private GCWearWJEQ(self: Model_WuJiang, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 0) {
			var len = data.readShort();
			for (let i = 0; i < len; i++) {
	...
	private GCHechengJY(self: Model_WuJiang, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 0) {
			var pos = data.readByte();
			var sid = data.readLong();
	...
	private GCUpJY(self: Model_WuJiang, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 0) {
			var pos = data.readByte();
			var sid = data.readLong();
	...
	private GCDecompose(self: Model_WuJiang, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 0) {
			var sid = data.readLong();
	...
	private GCChangeJob(self: Model_WuJiang, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 0) {
			var job = data.readByte();
			const shiZhuang = data.readInt();
	...
	/**升级神将之力 */
	private GCGetGodPower(self: Model_WuJiang, data: BaseBytes) {
	...
	private GC_GENERAL_SKILLUP(self: Model_WuJiang, data: BaseBytes) {
		let result = data.readByte();
		let id = data.readInt();
		let level = data.readInt();
		...
		if (result == 1) {
			let skillList = Model_player.voMine.skillList;
			let godskillCfg: Iherogodskill_211 = Config.herogodskill_211[id * 100 + level];
			for (let j = 0; j < skillList.length; j++) {
				let skillVo = skillList[j];
				let damage = 0;
				if (skillVo.type == 2 && godskillCfg) {
			...
			let arr: Array<Ihero_211> = Model_WuJiang.PuTongWujiang.concat();
			let len: number = arr.length;
			for (let i: number = 0; i < len; i++) {
				let cfg: Ihero_211 = arr[i];
	 ...
	 * @id 武将id
	 */
	public CG3501(id: number) {
		const bytes = this.getBytes();
		bytes.writeInt(id);
		this.sendSocket(3501, bytes);
	}
	private GC3502(self: Model_WuJiang, bytes: BaseBytes) {
		//I:武将idI:当前佩戴皮肤0没有[I:时装idI:星级]已经拥有时装id
		const id: number = bytes.readInt();
		const info = Model_WuJiang.shiZhuanDic[id] || (Model_WuJiang.shiZhuanDic[id] = <any>{});
		info.onSkinId = bytes.readInt();
		const len = bytes.readShort();
		const arr = [];
		info.ownSkinIds = [];
		let daojuDic = Model_WuJiang.itemShiZhuang;
		for (let i = 0; i < len; i++) {
			let sid = bytes.readInt();
			let lv = bytes.readInt();
			info.ownSkinIds.push({ skinId: sid, starLv: lv });
			let cfg = Config.sz_739[sid];
			let itemID = JSON.parse(cfg.jihuo)[0][1];
	 ...
	 * @id 时装id
	 */
	public CG3503(id: number, isJh: number = 0) {
		const bytes = this.getBytes();
		bytes.writeInt(id);
		this.isJH |= isJh;
		this.sendSocket(3503, bytes);
	}
	public isJH: number = 0;
	private GC3504(self: Model_WuJiang, bytes: BaseBytes) {
		// B:0成功1失败I:时装idI:时装星级
		const state = bytes.readByte();
		if (state == 0) {
			const skinId = bytes.readInt();
			const starLv = bytes.readInt();
			const info = Model_WuJiang.shiZhuanDic[Model_WuJiang.curSelWJId];
			const arr = info.ownSkinIds;
			for (let i = 0; i < arr.length; i++) {
				const skinInfo = arr[i];
	 ...
	 * @id 时装id
	 */
	public CG3505(wjid: number, skinId: number) {
		const bytes = this.getBytes();
		bytes.writeByte(wjid);
		bytes.writeInt(skinId);
		this.sendSocket(3505, bytes);
	}
	private GC3506(self: Model_WuJiang, bytes: BaseBytes) {
		//B:0成功1失败I:时装id
		const state = bytes.readByte();
		if (state == 0) {
			const skinId = bytes.readInt();
			...
			const voMine = Model_player.voMine;
			const skinIdToJob = skinId / 1000 >> 0;
	...
	/**3509拉取所有的时装数据 */
	public CG3509() { this.sendSocket(3509, this.getBytes()); }
	private GC3510(self: Model_WuJiang, bytes: BaseBytes) {
		for (let i = 0, len = bytes.readShort(); i < len; i++) {
			self.GC3502(self, bytes);
		}
	}

	public static sortWuJiang(a: Ihero_211, b: Ihero_211): number {
		let aQua = Model_WuJiang.getHeroQuality(a);
		let bQua = Model_WuJiang.getHeroQuality(b);
		if (aQua == bQua) {
	...
	public static sortWuJiang1(a: Ihero_211, b: Ihero_211): number {
		let aQua = Model_WuJiang.getHeroQuality(a);
		let bQua = Model_WuJiang.getHeroQuality(b);
		if (aQua == bQua) {
			return a.type - b.type;
	...
	public static getHeroQuality(v: Ihero_211): number {
		if (v == null) {
			return 1;
		}
		if (v.quality == null) {
	...
	static createWuJiangColorName(id) {
		let cfg = Config.hero_211[id];
	...
	public static checkOneKeyUp(): boolean {
		let count = Model_Bag.getItemCount(Model_WuJiang.DAN_LEVELUP)
		let exp = count * Model_WuJiang.DAN_EXP
		let herolv = Config.herolv_211[Model_WuJiang.jieShu]
	...
	public static checkSkill(id): boolean {
		let obj = Config.herolvskill_211[id];
		if (obj == null) {
			return false;
		}
		if (obj.next == 0) {//已满级
	...
	public static checkUpJie(): boolean {
		...
		//武将技能升级
		let len = Model_WuJiang.skillArr.length
		for (let i = 0; i < len; i++) {
			let id = Model_WuJiang.skillArr[i]
			if (Model_WuJiang.checkSkill(id)) {
				return true;
			}
		}
		if (Model_WuJiang.wuJWearArr().length > 0) {//可穿戴装备
			return true;
		}
		return false;
	}

	public static checkStarVo(wuJiang: Ihero_211): boolean {
		var star = Model_WuJiang.wuJiangStar[wuJiang.type];
		if (star && star >= wuJiang.star) {
			return false;
		}
		//升星道具
		let consume = ConfigHelp.SplitStr(wuJiang.activation)
	...
	public static checkUpStar(): boolean {
		//激活升星
		let len = Model_WuJiang.PuTongWujiang.length;
		for (let i = 0; i < len; i++) {
			let wuJiang = Model_WuJiang.PuTongWujiang[i];
		...
		let count = Model_Bag.getItemCount(Model_WuJiang.DAN_SHUXING)
	...
	//将印可合成 升阶
	public static checkJYinSyn(type: number): boolean {
		let open = Model_WuJiang.OPEN_GUAN[type - 20]
		if (GGlobal.modelguanxian.guanzhi < open) {//未开启
			return false;
		}
		let equipData = Model_player.voMine.equipData
		let ve: VoEquip = equipData[type];
		let next
		if (ve == null) {//合成
		...
		let composeArr: Array<any> = ConfigHelp.SplitStr(next.compose)
		let count = Model_Bag.getItemCount(Number(composeArr[0][1]));
		let needCount = Number(composeArr[0][2])
		if (count >= needCount) {
			return true;
		}
		return false;
	...
	//将印可穿戴 合成 升阶
	public static checkJYin(): boolean {
		for (let i = 0; i < 10; i++) {
		...
		//将印可穿戴
		let equipBag = Model_Bag.equipList
		let equipData = Model_player.voMine.equipData
		for (let i = 0; i < equipBag.length; i++) {
			let ve: VoEquip = equipBag[i];
			if (ve.type >= 20 && ve.type < 30) {
				let open = Model_WuJiang.OPEN_GUAN[ve.type - 20]
				if (GGlobal.modelguanxian.guanzhi < open) {//未开启
					continue;
				}
	...
	public static newItem: Array<VoItem> = [];
	public static addNewItem(getNewItem: Array<VoItem>): void {
		Model_WuJiang.newItem = Model_WuJiang.newItem.concat(getNewItem);
		Model_WuJiang.showNewItem();
	}

	public static showNewItem(): void {
		while (true) {
			if (Model_WuJiang.newItem.length == 0) {
				break;
			}
			let v = Model_WuJiang.newItem.shift();
			let sys = v.cfg.sys
			if (sys == UIConst.WU_JIANG) {//武将
				let wu: Ihero_211 = Model_WuJiang.newWuJiang[v.id]
	...
	static getFashionWith_JobKey(type) {
		if (!Model_WuJiang.fashionMap) {
			const temp = {};
			const lib = Config.sz_739;
			for (let key in lib) {
				const cfg = lib[key];
				const wjId = cfg.ID / 1000 >> 0;
				if (!temp[wjId]) {
					temp[wjId] = cfg;
				}
			}
	...
	/**单个武将时装 */
	public static SZCheck(wujiang: Ihero_211, szId?: number) {
		const lib = Config.sz_739;
		var target: Isz_739;
		var star_max = 0;
		if (szId) {
		...
		if (!this.shiZhuanDic || !this.shiZhuanDic[wujiang.type]) {
			...
			const arr = this.shiZhuanDic[wujiang.type].ownSkinIds;
			for (let i = 0; i < arr.length; i++) {
				var tempObj = arr[i];
		...
		if (target) star_max = target.shangxian;
	...
	/**所有时装 */
	public static SZcheckAll() {
		const arr = Model_WuJiang.wuJiangArr;
	...
	/**判断武将是否有时装展示 */
	private static judgeArr;
	public static hasShiZhuang(wjId: number) {
		if (!this.judgeArr) {
			const lib = Config.sz_739;
	...
	/**获取武将 */
	public static findWJ(id: number) {
		const arr = Model_WuJiang.wuJiangArr;
	...
	//	获取可以穿的武将装备
	public static wuJWearArr(): Array<VoEquip> {
		let arr: Array<VoEquip> = Model_Bag.filterEquips(Model_Bag.filterWuJEquip, null);
		let d = Model_player.voMine.equipData
		let sendArr = {};
		for (let i = 0; i < arr.length; i++) {
			let equ = arr[i];
			if (Model_WuJiang.jieShu < equ.jie) {//不可穿 武将
				continue;
			}
			let ownE = d[equ.type];
	...
	/**材料到英雄的映射 */
	private static matToWuJiang: { [key: number]: Ihero_211 } = {};
	/**通过激活材料(升星材料)判断使用该材料的武将是否已经满星 */
	public static isFullByMat(vo: VoItem) {
		if (this.matToWuJiang[vo.id]) {
			const data = this.matToWuJiang[vo.id];
			return this.wuJiangStar[data.type] >= data.star;
		} else {
			const datas = this.wuJiangArr;
			for (let i = 0; i < datas.length; i++) {
				const data = datas[i];
				let consume = ConfigHelp.SplitStr(data.activation);
	...
	/**获取已经激活的武将 */
	public static getActiveWUJiang(): Ihero_211[] {
		let data = Model_WuJiang.wuJiangArr;
		let temp: Ihero_211[] = [];
		for (let i = 0; i < data.length; i++) {
			let v = data[i];
			let start = Model_WuJiang.wuJiangStar[v.type]
			if (start) {
				temp.push(v);
				continue;
			}
	...
	/**通过品质获取所有该品质已激活的武将 */
	public static getWUjiangStar(type: number) {
		let data: Ihero_211[] = Model_WuJiang.getActiveWUJiang();
		let temp: Ihero_211[] = [];
		for (let i = 0; i < data.length; i++) {
			let v = data[i];
			if (v.pinzhi == type) {
	...
	/**通过品质获取该品质所有武将的总星级 */
	public static getAllWujiangStarByPinzhi(type: number) {
		let data = Model_WuJiang.getWUjiangStar(type);
		let index = 0;
		for (let i = 0; i < data.length; i++) {
			let v = data[i];
			let level = Model_WuJiang.wuJiangStar[v.type];
			index += level;
		}
		return index;
	}

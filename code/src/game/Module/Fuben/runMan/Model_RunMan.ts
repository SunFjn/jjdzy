class Model_RunMan extends BaseModel {
	...
	/**选择关卡难度挑战 B:难度*/
	public CG_BattleType(type): void {
		var bates = this.getBytes();
		bates.writeByte(type)
		this.sendSocket(1553, bates);
	}
	/**获取某类关卡奖励 B:关卡难度类型*/
	public CG_GetReward(type): void {
		var bates = this.getBytes();
		bates.writeByte(type)
		this.sendSocket(1555, bates);
	}
	/**扫荡过关斩将 B:副本难度*/
	public CG_OneKey(type): void {
		var bates = this.getBytes();
		bates.writeByte(type)
	...
	public static layerInfo: VoRunManInfo[] = [];
	// public static maxType: number = 0;//最高难度
	// public static rewardArr

	//过关斩将通关信息 [B:副本难度type1-4I:本日已通关idI:历史最高层id]
	public GC_OPENUI(self: Model_RunMan, data: BaseBytes): void {
		var len = data.readShort();
		// Model_RunMan.maxType = 0;
		for (let i = 0; i < len; i++) {
			let info: VoRunManInfo = new VoRunManInfo();
	...
	public static battleType

	//关卡挑战难度 B:0成功 1失败B:难度B:战斗结果0:失败，1：成功，2：以前端结果为准I:关卡id
	public GC_BATTLE_TYPE(self: Model_RunMan, data: BaseBytes): void {
	...
	public static dropArr//掉落奖励
	public static dropFirst//首通奖励
	public static dropHun:Vo_JiangHun//激活将魂
	//获取关卡奖励结果 B:0成功 1失败B:难度类型B:是否是第一次通过0不是1是I:关卡id[B:类型I:系统idI:数量]
	public GC_GETREWARD(self: Model_RunMan, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 0) {
	...
	public GC_ONEKEY(self: Model_RunMan, data: BaseBytes): void {
		var result = data.readByte();
		if (result == 0) {
			var type = data.readByte();
			var len = data.readShort();
	...
	private static _soulReard: any
	private static initSoulReward(): void {
		if (Model_RunMan._soulReard == null) {
			Model_RunMan._soulReard = {};
			for (let keys in Config.general_006) {
	...
	public static getSoulEqual(type, layer): any {
		Model_RunMan.initSoulReward();
		var soulArr = Model_RunMan._soulReard[type];
		for (let i = 0; i < soulArr.length; i++) {
			if (soulArr[i].actLayer == layer) {
	...
	public static getTypeName(v: number): string {
		switch (v) {
			case 1:
				return "普通"
			case 2:
	...
	public static getIsPass(type: number, layer: number): boolean {
		var info = Model_RunMan.layerInfo[type - 1]
		if (info == null) {
			return false;
		}
...
declare interface Igeneral_006 {
	actType;
	actLayer;
}

class Model_YJDQ extends BaseModel {
	public static curPass: number = 1001;
	public static type: number = 1;
	public static passMax: number;
	public static battleNum: number;
	public static buyNum: number;
	public static dataArr: Array<any> = [];
	public static rankData: Array<any> = [];
	public static battleRet: number = 0;
	public static rewardMax = 20;

	public static checkTabNotice(): boolean {
		let arr = Model_YJDQ.dataArr;
		let len = arr.length;
		for (let i = 0; i < len; i++) {
			if (Model_YJDQ.checkNoticeByType(i)) return true;
		}
		if (Model_YJDQ.battleNum > 0) return true;
		return false;
	}

	public static checkNoticeByType(type: number): boolean {
		if (type >= Model_YJDQ.dataArr.length) return false;
		let cfg = Config.yiqi_007[Math.floor(Model_YJDQ.dataArr[type][1] / Model_YJDQ.rewardMax + 1) * Model_YJDQ.rewardMax];
		if (cfg) {
	...
	/**1281 打开一骑当千界面   */
	public CG_OPEN_YJDQ(): void {
		let ba: BaseBytes = new BaseBytes();
		this.sendSocket(1281, ba);
	}

	/**1283 请求挑战 B:难度    */
	public CG_YJDQ_BATTLEBYTYPE(type: number): void {
		let ba: BaseBytes = new BaseBytes();
		ba.writeByte(type);
	...
	/**1285  胜利，打完当前一波怪      */
	public CG_YJDQ_SYSWIN(type: number): void {
		let ba: BaseBytes = new BaseBytes();
		ba.writeByte(type);
	...
	/**1287  购买挑战次数     */
	public CG_YJDQ_BUY_BATTLENUM(ct): void {
		let ba: BaseBytes = new BaseBytes();
		ba.writeByte(ct);
	...
	/**1289 领取首通奖励 B:难度     */
	public CG_YJDQ_DRAWREWARD(type: number): void {
		let ba: BaseBytes = new BaseBytes();
		ba.writeByte(type);
	...
	public GC_YJDQ_DRAWREWARD(self: Model_YJDQ, data: BaseBytes): void {
		let result = data.readByte();
		if (result == 1) {
			let pass = data.readInt();
			let type = Math.floor(pass / 1000);
	...
	public GC_YJDQ_BUY_BATTLENUM(self: Model_YJDQ, data: BaseBytes): void {
		let result = data.readByte();
		if (result == 1) {
	...
	public GC_YJDQ_SYSWIN(self: Model_YJDQ, data: BaseBytes): void {
		let pass = data.readInt();
		let result = data.readByte();
		let cfg = Config.yiqi_007[pass];
		if (result == 1 || result == 0 || result == 2) {
			...
			if (pass > Model_YJDQ.dataArr[cfg.type - 1][0]) Model_YJDQ.dataArr[cfg.type - 1][0] = pass;
			if (pass > Model_YJDQ.passMax) Model_YJDQ.passMax = pass;
			if (cfg.type == Model_YJDQ.type && Model_YJDQ.dataArr[cfg.type]) Model_YJDQ.type++;
			Model_YJDQ.curPass = pass;
			GGlobal.layerMgr.open(UIConst.FUBEN_YJDQ_WIN, 1);
		} else if (result == 4) {
			if (pass - 1 > Model_YJDQ.dataArr[cfg.type - 1][0]) Model_YJDQ.dataArr[cfg.type - 1][0] = pass - 1;
			if (pass - 1 > Model_YJDQ.passMax) Model_YJDQ.passMax = pass - 1;
	...
	public GC_YJDQ_BATTLEBYTYPE(self: Model_YJDQ, data: BaseBytes): void {
		let result = data.readByte();
		if (result == 0) {
	...
	public GC_OPEN_YJDQ(self: Model_YJDQ, data: BaseBytes): void {
		Model_YJDQ.isFirstOpen = true;
		let type = data.readByte();
		let passMax = data.readInt();
		for (let i = 0, len = data.readShort(); i < len; i++) {
			let type1: number = data.readByte();
			let todayPass = data.readInt();
			let rewardNum = data.readInt();
			if (rewardNum <= 0) rewardNum = type1 * 1000;

class Child_ShenJian_JiBan extends fairygui.GComponent {
	...
	private _conditionArr: Array<Vo_ShenJian>;
	private sysName = "神剑";

	public static createInstance(): Child_ShenJian_JiBan {
		return <Child_ShenJian_JiBan><any>(fairygui.UIPackage.createObject("role", "Child_BaoWu_JiBan"));
	}
	...
	private upPower(): void {
		...
		if (sf._jiBanArr) {
			for (let i = 0; i < sf._jiBanArr.length; i++) {
				...
				let suit = Config.swordsuit_216[ids];
				if (suit) {
	...
	private renderGrid(index: number, obj: fairygui.GObject): void {
		const sf = this;
		let v: ViewShenJianGrid = obj as ViewShenJianGrid
	...
	private selectdUpdate(suitId: number): void {
		...
		for (let i = 0; i < sf._jiBanArr.length; i++) {
			...
			let cfg = Config.swordsuit_216[v];

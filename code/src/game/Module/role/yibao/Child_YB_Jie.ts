class Child_YB_Jie extends fairygui.GComponent {
	...
	public static createInstance(): Child_YB_Jie {
		return <Child_YB_Jie><any>(fairygui.UIPackage.createObject("role", "Child_BaoWu_Jie"));
	}
	...
	public update(): void {
		...
		let exp = Model_BySys.sysExp(Model_BySys.YI_BAO);
		if (jieShu > 0) {
		...
		const cnt = Model_Bag.getItemCount(415007);
	...
	public getPowerSkill(): number {
		...
		if (skillArr) {
			for (let i = 0; i < skillArr.length; i++) {
				...
				let skill = Config.yblvskill_217[skillId];
	...
	private onEquip(): void {
		let s = this;
		let sendArr = Model_YiBao.yiBaoWearArr();
		let a = []
	...
	private onEquipTips(e: egret.Event) {
		...
		if (!v.vo) {
			let nitem = VoItem.create(410036);

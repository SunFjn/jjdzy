/**
 * 六道 分解界面
 */
class ChildSixWayFenJie extends UIPanelBase{
	public list: fairygui.GList;
	public lbName: fairygui.GTextField;
	public lbAttr: fairygui.GTextField;
	public lbLevel: fairygui.GTextField;
	public lbPower: fairygui.GTextField;
	public grid: VSixWayGrid;
	public labT: fairygui.GLabel
	public btnFenJ: Button0;
	public vres: ViewResource;
	public check1: fairygui.GButton;
	public check2: fairygui.GButton;
	public check3: fairygui.GButton;
	public check4: fairygui.GButton;
	public check5: fairygui.GButton;
	private _panelId:number = 0;

	public static URL: string = "ui://ehelf5bh11m1wz";

	private _checkArr: fairygui.GButton[];

	public static createInstance(): ChildSixWayFenJie {
		return <ChildSixWayFenJie><any>(fairygui.UIPackage.createObject("lunhui", "ChildSixWayFenJie"));
	}

	public constructor() {
		super();
		this.setSkin("lunhui", "lunhui_atlas0", "ChildSixWayFenJie");
	}

	protected constructFromXML(xml: any): void {
		let self = this;
		CommonManager.parseChildren(self, self);

		// self.list.callbackThisObj = self;
		// self.list.itemRenderer = self.renderHander;
		// self.list.setVirtual();
		// self._checkArr = []
		// for (let i = 0; i < 5; i++) {
		// 	self._checkArr.push(<fairygui.GButton><any>(self.getChild("check" + i)))
		// }
	}

	protected initView(): void {
		super.initView();

		let self = this;
		self.list.callbackThisObj = self;
		self.list.itemRenderer = self.renderHander;
		self.list.setVirtual();
	}

	protected onShown(): void {
		let s = this;
		s._panelId = s._args;
		s.vres.setType(0);
		s.btnFenJ.addClickListener(s.onFenJ, s);
		s._checkArr = [];
		for (let i = 0; i < 5; i++) {
			s["check" + (i + 1)].addClickListener(s.onCheck, s);
			if (i < 2) {
				s["check" + (i + 1)].selected = true
			} else {
				s["check" + (i + 1)].selected = false
			}
			s._checkArr.push(s["check" + (i + 1)]);
		}
		for (let i = 0; i < Model_LunHui.bagArr.length; i++) {
			let v = Model_LunHui.bagArr[i];
			if (v.type == 0 || v.pz < 5) {
				v.fenJ = 1;
			} else {
				v.fenJ = 0;
			}
		}
		s.list.addEventListener(fairygui.ItemEvent.CLICK, s.itemClick, s);
		GGlobal.control.listen(UIConst.SIXWAY_FENJIE, s.upFenJ, s);
		// GGlobal.socketMgr.registerReconnectHD("ChildBaZhenTuFenJ_onFenJ", Handler.create(GGlobal.modelBaZhenTu, GGlobal.modelBaZhenTu.CGOPENUI4401));
		s.update();
	}

	protected onHide(): void {
		let s = this;
		s.btnFenJ.removeClickListener(s.onFenJ, s);
		for (let i = 0; i < 5; i++) {
			s._checkArr[i].removeClickListener(s.onCheck, s);
		}
		// s.grid.showEff(false)
		s.list.numItems = 0;
		s.list.removeEventListener(fairygui.ItemEvent.CLICK, s.itemClick, s);
		GGlobal.control.remove(UIConst.SIXWAY_FENJIE, s.upFenJ, s);
		// GGlobal.socketMgr.removeReconnectHD("ChildBaZhenTuFenJ_onFenJ");
		if(s._panelId == UIConst.SIXWAY)
		{
			GGlobal.layerMgr.open(UIConst.SIXWAY, 2);
		}else{
			GGlobal.layerMgr.open(UIConst.SIXWAY_YINJI, Model_LunHui.type);
		}
	}

	private onCheck(e: egret.TouchEvent) {
		let checkBtn: fairygui.GButton = e.currentTarget as fairygui.GButton
		if (checkBtn.id == this._checkArr[3].id || checkBtn.id == this._checkArr[4].id) {
			if (checkBtn.selected) {
				ViewAlert.show(`当前选中了珍稀印记，确认分解？`, Handler.create(this, this.doCheckSure, [checkBtn], true),
					ViewAlert.OKANDCANCEL, "确认", "取消", Handler.create(this, this.doCheckCancel, [checkBtn], true), false, Handler.create(this, this.doCheckCancel, [checkBtn], true));
				return;
			}
		}
		this.doCheck();
	}

	private doCheckSure(check: fairygui.GButton) {
		check.selected = true
		this.doCheck();
	}

	private doCheckCancel(check: fairygui.GButton) {
		check.selected = false
		this.doCheck();
	}

	private doCheck() {
		let s = this;
		let model = GGlobal.modellh;
		let arr = [];
		for (let i = 0; i < 5; i++) {
			if (s._checkArr[i].selected) {
				if (i == 0) {
					arr.push(2)
					arr.push(3)
				}
				else if (i == 4) {
					arr.push(8)
				}
				else {
					arr.push(i + 3)
				}
			}
		}
		for (let i = 0; i < Model_LunHui.bagArr.length; i++) {
			let v = Model_LunHui.bagArr[i];
			let isFj = 0
			for (let j = 0; j < arr.length; j++) {
				if (v.pz == arr[j]) {
					isFj = 1;
					break
				}
			}
			v.fenJ = isFj
		}
		model.notify(Model_LunHui.CHECKED);
		s.upYinji();
	}

	private renderHander(index: number, obj: fairygui.GObject): void {
		var gird: VSixWayGridFenJie = obj as VSixWayGridFenJie;
		gird.grid.isShowEff = true;
		gird.voFenJ = this._showArr[index];
	}

	private itemClick(e: fairygui.ItemEvent): void {
		var gird: VSixWayGridFenJie = e.itemObject as VSixWayGridFenJie;
		this.upSelect(gird.vo);
		gird.onCheck()
		this.upYinji();
	}

	private _showArr;
	private update() {
		let s = this;
		s._showArr = [];
		for (let i = 0; i < Model_LunHui.bagArr.length; i++) {
			s._showArr.push(Model_LunHui.bagArr[i]);
		}
		s._showArr.sort(s.sortFunc);
		s.list.numItems = s._showArr.length;
		if (Model_LunHui.bagArr.length > 0) {
			s.list.scrollToView(0);
			s.list.selectedIndex = 0;
		}
		s.upSelect(s._showArr[0])
		s.upYinji();
		s.btnFenJ.checkNotice = Model_BaZhenTu.checkFenJ();
	}

	private sortFunc(a: VoSixWay, b: VoSixWay) {
		if (a.pz != b.pz) {
			return a.pz - b.pz;
		}
		if (a.lv != b.lv) {
			return a.lv - b.lv;
		}
		if (a.star != b.star) {
			return a.star - b.star;
		}
		return a.id - b.id;
	}

	private _selectVo: VoSixWay;
	private upSelect(v: VoSixWay) {
		let self = this;
		self._selectVo = v;
		self.grid.isShowEff = true
		self.grid.vo = v;
		if (v && v.cfg) {
			self.lbName.text = ConfigHelp.createColorName(v.name, v.pz);
			self.lbLevel.text = "Lv." + v.lv + "/" + v.maxLv;
			self.lbPower.text = "战力：" + v.power;
			self.lbAttr.text = ConfigHelp.attrStringQian(v.attr, "+", null, "#15f234");
		} else {
			self.lbPower.text = ""
			self.lbName.text = ""
			self.lbLevel.text = ""
			self.lbAttr.text = ""
		}
	}

	private idxArr = [0, 0, 0, 0, 1, 2, 3, 3, 4];
	private onFenJ() {
		if (!TimeUitl.cool("ChildSixWayFenJie_onFenJ", 200)) {
			return;
		}
		let s = this;
		let loacIndex = 0;
		let arr = []
		let tyfenJ = true;
		for (let i = 0; i < Model_LunHui.bagArr.length; i++) {
			let v:VoSixWay = Model_LunHui.bagArr[i];
			let idx = s.idxArr[v.cfg.pz]
			if (v.fenJ == 1) {
				arr.push(v.pos);
				//分解的  check未选中
				if (tyfenJ && s._checkArr[idx].selected == false) {
					tyfenJ = false;
				}
			} else {
				//不分解  check选中了
				if (tyfenJ && s._checkArr[idx].selected == true) {
					tyfenJ = false;
				}
			}
		}
		if (arr.length == 0) {
			ViewCommonWarn.text("未选中任何印记");
			return;
		}
		if (tyfenJ) {
			let arrCheck = []
			for (let i = 0; i < s._checkArr.length; i++) {
				let ck = this._checkArr[i]
				if (ck.selected) {
					if (i == 0) {
						arrCheck.push(2)
						arrCheck.push(3)
					}
					else if (i == 4) {
						arrCheck.push(8)
					}
					else {
						arrCheck.push(i + 3)
					}
				}
			}
			GGlobal.modellh.CG_FENJIEBYTYPE(arrCheck);
		} else {
			GGlobal.modellh.CG_RESOLVE(arr);
		}
	}

	private upYinji() {
		let s = this;
		let m = GGlobal.modellh;
		let total = 0;
		for (let i = 0; i < Model_LunHui.bagArr.length; i++) {
			let v = Model_LunHui.bagArr[i];
			if (v.fenJ == 1) {
				total += v.fjNum;
			}
		}
		let str = ConfigHelp.numToStr(total);
		ImageLoader.instance.loader(Enum_Path.ICON70_URL + "29.png", s.vres.getChild("icon").asLoader);
		s.vres.text = str;
		s.vres.color = Color.WHITEINT;
	}

	private upFenJ() {
		for (let i = 0; i < 5; i++) {
			this._checkArr[i].selected = false;
		}
		this.update();
	}
}
class Model_GroupB814 extends BaseModel {
	public constructor() {
		super();
	}
	...
	*/
	private GC_INIT(s: Model_GroupB814, d: BaseBytes) {
		s.charge = d.readInt();
		s.buyNum = d.readInt();
		let len = d.readShort();
		s.buyArr = [];
	 ...
	 * 奖励发生变化 I:奖励序号B:奖励状态
	*/
	private GC_REWARD_CHARGE(s: Model_GroupB814, d: BaseBytes) {
		let id = d.readInt();
	 ...
	 * 今日首冲人数 I:今日首冲人数变化
	*/
	private GC_FRIST_RENUM(s: Model_GroupB814, d: BaseBytes) {
		s.buyNum = d.readInt();
		let len = s.buyArr.length;
		let change = false;
		for (let a = 0; a < len; a++) {

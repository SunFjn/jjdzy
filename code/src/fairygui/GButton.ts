...
module fairygui {
    ...
    export class GButton extends GComponent {
        ...
        private static _hasLoadSound = 0;
        protected __click(evt: egret.TouchEvent): void {
            if (this._sound) {
                // var pi: PackageItem = UIPackage.getItemByURL(this._sound);
                // if (pi) {
                //     pi.file = this._sound;
                // var sound: egret.Sound = <egret.Sound>pi.owner.getItemAsset(pi);
                var sound: egret.Sound = <egret.Sound>RES.getRes(this._sound);
                if (sound) {
                    if(SoundManager.getInstance().EFF){
                        GRoot.inst.playOneShotSound(sound, UIConfig.buttonSoundVolumeScale);
                    }
                } else {

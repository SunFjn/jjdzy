...
public class DailyDirectBuyActCache extends AbsServerEvent {
	/** 每日直购按期数分类配置表，第一层key:期数，第二层key:天数 **/
	private static Map<Integer, Map<Integer, List<Struct_mrzg2_256>>> dailyDirectBuyConfigMap = new HashMap<>();
	...
	public static Map<Integer, Map<Integer, List<Struct_mrzg2_256>>> getDailyDirectBuyConfigMap() {
	...
	public void initExcel() throws RunServerException {
		...
		List<Struct_mrzg2_256> sortList = Config_mrzg2_256.getIns().getSortList();
		for (int i = 0; i < sortList.size(); i++) {
			Struct_mrzg2_256 struct_mrzg2_256 = sortList.get(i);
			int qs = struct_mrzg2_256.getQs();
			Map<Integer, List<Struct_mrzg2_256>> map = dailyDirectBuyConfigMap.get(qs);
			if (map == null) {
				map = new HashMap<>();
			...
			List<Struct_mrzg2_256> list = map.get(day);
			if (list == null) {
				list = new ArrayList<>();
				map.put(day, list);
			}
			list.add(struct_mrzg2_256);
		}

		for (Map<Integer, List<Struct_mrzg2_256>> map : dailyDirectBuyConfigMap.values()) {
			for (List<Struct_mrzg2_256> list : map.values()) {
				Collections.sort(list, new ConfigRankModelAct());
			}
		}
		...
		for (int i = 0; i < size; i++) {
			...
			if (struct_mrzgmb_256.getId() == ActivitySysId.ACT_DAILYDIRECTBUY) {

...
public class CrossShaoZhuQiYuanRankActSysCache extends AbsServerEvent {
	...
	private static Map<Integer, CrossShaoZhuQiYuanRankActCache> crossCache;

	public static Map<Integer, Map<Integer, Struct_wszwxsxspm_325>> getRankConfigMap() {
		return rankConfigMap;
	}

	public static Map<Integer, CrossShaoZhuQiYuanRankActCache> getCrossCache() {
		return crossCache;
	}

	public static CrossShaoZhuQiYuanRankActCache getRankCache(Channel channel) {
		int partId = CrossCache.getPartId(channel);
		CrossShaoZhuQiYuanRankActCache rankCache = crossCache.get(partId);
		if (rankCache == null) {
			rankCache = new CrossShaoZhuQiYuanRankActCache();
	...
	public static int getQs(Channel channel) {
		CrossShaoZhuQiYuanRankActCache rankCache = getRankCache(channel);
	...
	public static TreeSet<ShaoZhuQiYuanRankActModel> getRankTreeSet(Channel channel) {
		CrossShaoZhuQiYuanRankActCache rankCache = getRankCache(channel);
		return rankCache.getRankTreeSet();
	}

	public static void setRankTreeSet(Channel channel, TreeSet<ShaoZhuQiYuanRankActModel> rankTreeSet) {
		CrossShaoZhuQiYuanRankActCache rankCache = getRankCache(channel);
		rankCache.setRankTreeSet(rankTreeSet);
	}

	public static List<ShaoZhuQiYuanRankActModel> getTempRankList(Channel channel) {
		CrossShaoZhuQiYuanRankActCache rankCache = getRankCache(channel);
		return rankCache.getTempRankList();
	}

	public static void setTempRankList(Channel channel, List<ShaoZhuQiYuanRankActModel> tempRankList) {
		CrossShaoZhuQiYuanRankActCache rankCache = getRankCache(channel);
	...
	public void startServer() throws RunServerException {
		...
		String content = "";
		try {
			GlobalData globalData = GlobalCache.getGlobalData(GlobalConst.CROSS_SHAOZHU_QIYUANRANK_ACT_CEN);
	...
	public void initExcel() throws RunServerException {
		...
		for (Struct_wszwxsxspm_325 struct_wszwxsxspm_325 : sortList) {
			if (struct_wszwxsxspm_325.getXtid() == ActivitySysId.SHAOZHU_QIYUANRANK_ACT) {

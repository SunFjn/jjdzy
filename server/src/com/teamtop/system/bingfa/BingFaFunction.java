...
public class BingFaFunction {
	
	private static BingFaFunction ins;
	public static BingFaFunction getIns(){
		if(ins == null) {
			ins = new BingFaFunction();
		}
		return ins;
	}
	 ...
	 * @return
	 */
	public int getBingfaTotelStr(Hero hero) {
		int score=0;
	...
	public int getBingfaStrByid(Hero hero,int index) {
		int score=0;
		FinalFightAttr finalAttr = new FinalFightAttr();
		FightAttr attr = new FightAttr();
		...
		int pingzhi=0;
		if (Config_book_213.getIns().get(bingFaModel.getIndex())!=null) {
			//CommonUtil.arrChargeMap(Config_book_213.getIns().get(bingFaModel.getIndex()).getAttr(), attrMap);
			pingzhi=Config_book_213.getIns().get(bingFaModel.getIndex()).getPin();
		}
		//升星
		int starindex=pingzhi*1000+bingFaModel.getStar();
		Struct_bookstar_213 struct_bookstar_213 = Config_bookstar_213.getIns().get(starindex);
		...
		//进阶
		/*if (Config_booklv_213.getIns().get(bingFa.getJieLv())!=null) {
			CommonUtil.arrChargeMap(Config_booklv_213.getIns().get(bingFa.getJieLv()).getAttr(), attrMap);
		}*/
		long[][] data=CommonUtil.mapToArr(attrMap);
		FightCalcFunction.setFightValue(data, attr);
		FightCalcFunction.calcEquipAttr(finalAttr, attr,0);

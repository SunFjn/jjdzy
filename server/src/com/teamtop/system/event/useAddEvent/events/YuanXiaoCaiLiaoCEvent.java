...
public class YuanXiaoCaiLiaoCEvent extends AbsUseAddEvent {
	...
	public long use(Hero hero, int num, int id, int reason) {
		...
		long SoulFire =yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_3) - num;
		return SoulFire;
	}
	...
	public boolean canAdd(Hero hero, int num, int id) {
		...
		return yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_3) + num <= HeroConst.MAX_MONEY;
	}
	...
	public long add(Hero hero, int num, int id) {
		...
		addCaiLiao.put(YuanXiaoConst.CAILIAO_3, num);
		YuanXiaoCrossIO.getIns().LTCaddcailiao(yuanXiaoLocal, addCaiLiao);
		
		return yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_3);
	}
	...
	public void flowRec(Hero hero, int num, int id, boolean add, int reason) {
		try {
			...
			FlowHeroEvent.addMoneyFlow(hero.getId(), hero.getLevel(), YuanXiaoConst.CAILIAO_3,
					yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_3), num, reason, hero.getZoneid(), pf, usesys, addFlag,
	...
	public long addHuobi(Hero hero, long num) {
		...
		HashMap<Integer, Integer> addCaiLiao=new HashMap<>();
		addCaiLiao.put(YuanXiaoConst.CAILIAO_3, (int)num);
		YuanXiaoCrossIO.getIns().LTCaddcailiao(yuanXiaoLocal, addCaiLiao);
		return yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_3);
	}

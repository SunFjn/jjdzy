...
/**
 * 活动材料 不能调用使用方法  只容许在本活动内使用
 * @author Administrator
 *
 */
public class YuanXiaoCaiLiaoAEvent extends AbsUseAddEvent {
	...
	public long use(Hero hero, int num, int id, int reason) {
		...
		long SoulFire =yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_1) - num;
		return SoulFire;
	}
	...
	public boolean canAdd(Hero hero, int num, int id) {
		...
		return yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_1) + num <= HeroConst.MAX_MONEY;
	}
	...
	public long add(Hero hero, int num, int id) {
		...
		addCaiLiao.put(YuanXiaoConst.CAILIAO_1, num);
		YuanXiaoCrossIO.getIns().LTCaddcailiao(yuanXiaoLocal, addCaiLiao);
		
		return yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_1);
	}
	...
	public void flowRec(Hero hero, int num, int id, boolean add, int reason) {
		try {
			...
			FlowHeroEvent.addMoneyFlow(hero.getId(), hero.getLevel(), YuanXiaoConst.CAILIAO_1,
					yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_1), num, reason, hero.getZoneid(), pf, usesys, addFlag,
	...
	public long addHuobi(Hero hero, long num) {
		...
		HashMap<Integer, Integer> addCaiLiao=new HashMap<>();
		addCaiLiao.put(YuanXiaoConst.CAILIAO_1, (int)num);
		YuanXiaoCrossIO.getIns().LTCaddcailiao(yuanXiaoLocal, addCaiLiao);
		return yuanXiaoLocal.getCailiaoMap().get(YuanXiaoConst.CAILIAO_1);
	}

...
public class CrossShaoZhuQiYuanRankCL {
	private static volatile CrossShaoZhuQiYuanRankCL ins = null;

	public static CrossShaoZhuQiYuanRankCL getIns() {
		if (ins == null) {
			synchronized (CrossShaoZhuQiYuanRankCL.class) {
				if (ins == null) {
					ins = new CrossShaoZhuQiYuanRankCL();
				}
			}
		}
		return ins;
	...
	private CrossShaoZhuQiYuanRankCL() {
	}
		...
		OpTaskExecutorService.PublicOrderService.execute(new RankingOpTaskRunnable() {
			...
			public void run() {
				...
				try {
					...
					if (size > 0) {
						crossData.putObject(CrossShaoZhuQiYuanRankEnum.rankTreeSet, rankTreeSet);
					}
					int beginTime = CrossShaoZhuQiYuanRankSysCache.getBeginTime(channel);
					crossData.putObject(CrossShaoZhuQiYuanRankEnum.beginTime, beginTime);
					int endTime = CrossShaoZhuQiYuanRankSysCache.getEndTime(channel);
					crossData.putObject(CrossShaoZhuQiYuanRankEnum.endTime, endTime);
					LogTool.info("CrossShaoZhuQiYuanRankCL:" + channel, this);
					NettyWrite.writeXData(channel, CrossConst.CROSS_SHAOZHU_QIYUANRANK_CONNSENDRANK_CL, crossData,
							new Callback() {
								...
								public void dataReci(Channel channel, CrossData crossData) {
									...
									try {
										...
										Integer serverOpenTime = crossData
												.getObject(CrossShaoZhuQiYuanRankEnum.serverOpenTime, Integer.class);
										Map<Integer, Integer> serverOpenTimeMap = CrossShaoZhuQiYuanRankSysCache
												.getServerOpenTimeMap(channel);
										serverOpenTimeMap.put(zoneId, serverOpenTime);
									} catch (Exception e) {
										// TODO: handle exception
										LogTool.error(e, this,
												"CrossShaoZhuQiYuanRankCL connEvent dataReci Exception!");
									}
								}
							});
				} catch (Exception e) {
					LogTool.error(e, this, "CrossShaoZhuQiYuanRankCL connEvent Exception! zoneid:" + zoneid);
				}
			}
	...
	public void checkOpenFirstUse() {
		...
		try {
			...
			for (Struct_hdfl_012 struct_hdfl_012 : sortList) {
				if (struct_hdfl_012.getId() != SystemIdConst.SHAOZHU_QIYUANRANK) {
					continue;
				}
				...
				for (Entry<Integer, Integer> entry : firstServerOpenTimeMap.entrySet()) {
					...
					int openTime = ShaoZhuQiYuanRankFunction.getIns().getOpenDaysTime(firstServerOpenTime, open, false);
					int endTime = ShaoZhuQiYuanRankFunction.getIns().getOpenDaysTime(firstServerOpenTime, end, true);
					int todayZeroTimeReturnInt = TimeDateUtil.getTodayZeroTimeReturnInt();
					if (todayZeroTimeReturnInt == openTime) {
						CrossShaoZhuQiYuanRankSysCache.setBeginTime(partId, openTime);
						CrossShaoZhuQiYuanRankSysCache.setEndTime(partId, endTime);
			...
			Map<Integer, CrossShaoZhuQiYuanRankCache> rankCacheMapByPartId = CrossShaoZhuQiYuanRankSysCache
			...
			LogTool.error(e, this,
					"CrossShaoZhuQiYuanRankCL checkOpenFirstUse Exception! firstZoneId:" + firstZoneId
	...
	public void syncTimeToLocal(int partId, int beginTime, int endTime) {
		...
		for (Channel channel1 : channelMap.keySet()) {
			...
			crossData.putObject(CrossShaoZhuQiYuanRankEnum.beginTime, beginTime);
			crossData.putObject(CrossShaoZhuQiYuanRankEnum.endTime, endTime);
			NettyWrite.writeXData(channel1, CrossConst.CROSS_SHAOZHU_QIYUANRANK_SYNCTIME_CL, crossData, new Callback() {
				@Override
				public void dataReci(Channel channel, CrossData crossData) {
					// TODO Auto-generated method stub
					...
					for (int i = 0; i < size; i++) {
						...
						LogTool.info("CrossShaoZhuQiYuanRankCL syncTimeToLocal zoneid:" + zoneid, this);
					}
				}
			});
		}
	 ...
	 */
	public void syncRankToLocal(Channel channel, CrossShaoZhuQiYuanRankModel addModel) {
		...
		for (Channel channel1 : channelToZoneid.keySet()) {
			CrossData crossData = new CrossData();
			crossData.putObject(CrossShaoZhuQiYuanRankEnum.addUpdateRankModel, addModel);
			NettyWrite.writeXData(channel1, CrossConst.CROSS_SHAOZHU_QIYUANRANK_ADDUPDATERANK_CL, crossData);
		}
	}
	...
	public void sendMailAwardToLocal() {
		OpTaskExecutorService.PublicOrderService.execute(new RankingOpTaskRunnable() {
			...
			public void run() {
				...
				Integer partId = 0;
				try {
					for (Entry<Integer, CrossShaoZhuQiYuanRankCache> entry : rankCacheMapByPartId.entrySet()) {
						partId = entry.getKey();
						CrossShaoZhuQiYuanRankCache rankCache = entry.getValue();
						if (!ShaoZhuQiYuanRankFunction.getIns().isEndCen(partId)) {
							continue;
						}

						if (rankCache.getSendAwardSet().contains(SystemIdConst.SHAOZHU_QIYUANRANK)) {
							continue;
						}
						...
						Map<Channel, List<CrossShaoZhuQiYuanRankModel>> awardByChannelMap = awardHandle(rankTreeSet);
						for (Channel channel1 : channelMap.keySet()) {
							List<CrossShaoZhuQiYuanRankModel> awardList = awardByChannelMap.get(channel1);
							try {
								if (awardList == null || awardList.size() == 0) {
									continue;
								}
								CrossData crossData = new CrossData();
								crossData.putObject(CrossShaoZhuQiYuanRankEnum.awardList, awardList);
								NettyWrite.writeXData(channel1, CrossConst.CROSS_SHAOZHU_QIYUANRANK_SENDMAILAWARD_CL,
										crossData);
							} catch (Exception e) {
								// TODO: handle exception
						...
						rankCache.getSendAwardSet().add(SystemIdConst.SHAOZHU_QIYUANRANK);
					}
					String content = JSON.toJSONString(rankCacheMapByPartId);
	 ...
	 * @return
	 */
	public Map<Channel, List<CrossShaoZhuQiYuanRankModel>> awardHandle(TreeSet<CrossShaoZhuQiYuanRankModel> treeSet) {
		Map<Channel, List<CrossShaoZhuQiYuanRankModel>> awardByChannelMap = new HashMap<>();
		int rank = 1;
		for (CrossShaoZhuQiYuanRankModel rankModel : treeSet) {
			...
			if (channel == null) {
				LogTool.warn("CrossShaoZhuQiYuanRankCL awardHandle dataReci Exception! hid:" + hid, this);
				continue;
			}
			List<CrossShaoZhuQiYuanRankModel> awardList = awardByChannelMap.get(channel);
			if (awardList == null) {

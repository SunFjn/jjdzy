...
public class SaintMonsterWashSysEvent extends AbsSystemEvent {

	private static SaintMonsterWashSysEvent ins;

	private SaintMonsterWashSysEvent() {
		// TODO Auto-generated constructor stub
	}

	public static synchronized SaintMonsterWashSysEvent getIns() {
		if (ins == null) {
			ins = new SaintMonsterWashSysEvent();
		}
		return ins;
	}
	...
	public void login(Hero hero) {
		...
		if(uid > 0) {
			...
			if(!(systemModel instanceof SaintMonsterWash)) {
				Map<Integer, AbsOpenDaysSystemModel> tempMap = heroOpenDaysSysData.getTempMap();
				AbsOpenDaysSystemModel systemModel2 = tempMap.get(16);
				if(systemModel2!=null) {						
					heroOpenDaysSysData.getOpSysDataMap().put(16, systemModel2);
				}
			}
		}
		boolean redPoint = SaintMonsterWashFunction.getIns().checkRedPoint(hero);
		if (redPoint) {
			RedPointFunction.getIns().addLoginRedPoint(hero, SystemIdConst.SAINT_MONSTER_WASH, RedPointConst.RED_1,

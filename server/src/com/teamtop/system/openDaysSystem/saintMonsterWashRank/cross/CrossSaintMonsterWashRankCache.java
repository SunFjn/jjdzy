...
public class CrossSaintMonsterWashRankCache extends AbsServerEvent {

	// key partId
	private static Map<Integer, List<SaintMonsterWashRankModel>> rankMap = new HashMap<>();

	public static Map<Integer, Integer> sendRewardMap = new HashMap<>();
	/** 活动结束时间 */
	...
	public static void setRankMap(Map<Integer, List<SaintMonsterWashRankModel>> rankMap) {
		CrossSaintMonsterWashRankCache.rankMap = rankMap;
	}

	public static Map<Integer, Integer> getSendRewardMap() {
		return sendRewardMap;
	}

	public static void setSendRewardMap(Map<Integer, Integer> sendRewardMap) {
		CrossSaintMonsterWashRankCache.sendRewardMap = sendRewardMap;
	}
	...
	public static void setEndTimeMap(Map<Integer, Integer> endTimeMap) {
		CrossSaintMonsterWashRankCache.endTimeMap = endTimeMap;
	}
	...
	public void startServer() throws RunServerException {
		...
		try {
			GlobalData globalData = GlobalCache.getGlobalData(GlobalConst.SAINT_MONSTER_WASH_RANK_SENDREWARD);
			String content = globalData.getContent();
			if (content == null || content.equals("") || content.equals("{}")) {
			} else {
				Type type = new TypeReference<Map<Integer, Integer>>() {
				}.getType();
				Map<Integer, Integer> sendRewardMap = JSONObject.parseObject(content, type);
				setSendRewardMap(sendRewardMap);
			}
		} catch (Exception e) {
			LogTool.error(e, CrossSaintMonsterWashRankCache.class, "CrossSaintMonsterWashRankCache startServer wrong");
			throw new RunServerException(e, "");
	...
	public void shutdownServer() {
		...
		try {
			GlobalData globalData = GlobalCache.getGlobalData(GlobalConst.SAINT_MONSTER_WASH_RANK_SENDREWARD);
			globalData.setContent(JSON.toJSONString(sendRewardMap));
			GlobalCache.doSync(globalData);
		} catch (Exception e) {
			LogTool.error(e, CrossSaintMonsterWashRankCache.class,
					"CrossSaintMonsterWashRankCache shutdownServer wrong");
